...
- `ifname` 由 user 模块在刷新用户 MAC 时同步更新，规格上依赖 `CONFIG_NATFLOW_PATH` 且仅在 path 运行时启用时生效。优先使用当前连接 path 缓存中与包方向相反的 `rroute[!dir].outdev`；route 尚未学习或已经失效时，回退到内核接收路径保存在 `skb->skb_iif` 中的原始入接口索引，在 RCU 保护下解析设备并复制当前名称。`skb_iif` 和 `dev_get_by_index_rcu()` 在仓库当前兼容目标内核中均可直接使用，不需要版本分支。该方式不在 path 的广播、组播或 ICMP 早退路径执行额外用户查找；没有 conntrack/user 刷新上下文的 ARP 等流量不会单独更新 ifname。
