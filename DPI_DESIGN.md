...
3. `urllogger`、`hostacl`、`urllogger_store` 已经是用户可见 ABI，直接改名会破坏部署。
...
| `natflow_l7_url.c/.h` 或保留拆薄后的 `natflow_urllogger.c/.h` | legacy URL consumer，消费 L7 packet view，保留 URL store、CSV 输出、Host ACL、302/RST 动作和 `urllogger_store` sysctl 资源。 |
...
- 当前 HTTP Host/URI、TLS SNI、QUIC v1 Initial SNI parser API 已阶段性迁移到 `natflow_l7.c`；TCP TLS SNI cache、QUIC cache、QUIC crypto ctx 和 QUIC UDP packet producer 已迁移到 `natflow_l7` 生命周期；legacy URL consumer 仍持有 TCP HTTP/TLS packet parsing、URL record 分配、Host ACL、队列输出和 ACL 回复策略。
...
- 保持现有 URL logger、Host ACL、sysctl、CSV 输出和默认行为兼容。
...
| ABI | 兼容要求 |
| --- | --- |
| `/dev/urllogger_queue` | 保持 CSV 字段、escaping、老化/合并、小 buffer 行为和 `clear` 命令。 |
...
- `CONFIG_NATFLOW_URLLOGGER`：启用 legacy URL consumer、Host ACL、`/dev/urllogger_queue`、`/dev/hostacl_ctl` 和 `urllogger_store` sysctl。
...
6. DPI consumer 默认 `enable=0`，只初始化控制设备和最小状态；M1 阶段 protocol-only hook 暂时保持在 `natflow_dpi.c` 内独立注册和退出。
...
- context 注册成功后才能让 owner bit 长期存在；注册失败必须写 error reason 并清 bit。
...
- read 输出必须包含 abi version、state、generation、ruleset bytes、active contexts、event lost、reason counters 和 enable 状态。

### 12.2 `/dev/natflow_dpi_queue`

DPI event 使用版本化二进制记录，不复用 `/dev/urllogger_queue` CSV。
...
legacy consumer 的目标是行为兼容，不是顺手修语义：

- `/dev/urllogger_queue` CSV 格式保持不变。
...
| 资源 | 初始上限 | 耗尽行为 |
| --- | --- | --- |
| `natflow_t` DPI 固定结果 | 4 B logical/flow | session 初始化失败则不分类。 |
| active L7 context | 256 个 | `CACHE_FULL`，fail-open。 |
...
| inspect wall clock | 4 秒 | `TIME_BUDGET`。 |
...
- 已完成阶段性迁移：HTTP Host、TLS SNI、QUIC Initial/CRYPTO/SNI 解析进入 `natflow_l7` feature core，TCP TLS SNI cache、QUIC cache、QUIC crypto ctx 和 QUIC UDP packet producer 迁入 `natflow_l7` 生命周期，legacy URL、Host ACL 和 DPI 继续保持原有外部 ABI；HTTP/TLS/QUIC consumer fan-out 已改为消费 `natflow_l7_host_view`，调用点不再分别散落 URL flags、DPI source 和 ACL 回复策略常量。
...
- `/dev/urllogger_queue` 字节级 CSV 回归。
...
- `CONFIG_NATFLOW_URLLOGGER_LOCAL_IN` 回归。
