...
1. MVP 不再限制为 HTTP/TLS/QUIC 域名映射；MVP 必须先建立 detector 框架、fast-path gate、终态和事件 ABI。首批 detector 包含 HTTP/1 Host、TLS ClientHello SNI、QUIC v1 Initial SNI，以及少量非 HTTP/TLS/QUIC 的高确定性 detector。具体协议集按 7.4 节分级进入，nDPI 参考和可实现清单见 7.5 节，不把 nDPI 全量协议一次搬入内核。
...
10. DNS snooping、JA3/JA4、任意 payload offset、User-Agent、HTTP path 组合规则、静态 IP fallback 和应用策略执行不进入 MVP。非 HTTP/TLS/QUIC 识别只通过显式列入的 detector 进入，且每个 detector 必须独立给出方向、包数、字节数、误判模型和资源预算。M1 拆成 M1a gate/ABI、M1b HTTP/TLS/QUIC+domain、M1c 首批非 HTTP detector、M1d 第二批 A 级 detector，只有真实命中率和性能数据证明需要后，才扩大协议集或开放 enforce。
...
| B | FTP、SMTP、POP3、IMAP、SIP、RTSP、MQTT、RESP、MySQL、PostgreSQL、RDP、SMB | 明文命令或二进制握手明确，但可能依赖 server-first 或 STARTTLS/升级路径 | 先 audit-only，覆盖率和误判稳定后再允许 policy |
| C | OpenVPN、SoftEther、Kerberos、RTP/RTCP、游戏/私有 TCP/UDP、代理/VPN 变体 | 需要多包方向模式、弱 magic、端口上下文或复杂状态；误判风险更高 | 默认只输出 protocol-only 或 hint，不用于阻断 |
...
本节基于本机 `/home/ubuntu/nDPI` 当前源码抽取，主要参考：

- `ndpi_protocol_detection_features.md`：当前工作区静态生成的协议/应用特征索引，说明 nDPI 每项识别来自 DPI detector、域名、IP 网段、TLS 证书还是复合协议。
- `src/include/ndpi_protocol_ids.h`：nDPI 的 `NDPI_PROTOCOL_*` 名称，仅作为参考名；Natflow UAPI 不复制这些枚举值。
- `src/lib/protocols/*.c`：每个协议 detector 的真实最小证据、方向阶段、包数和误判风险。
- `src/lib/ndpi_content_match.c.inc` 和 `src/lib/inc_generated/*_match.c.inc`：`host_match[]`、生成域名表、IP 网段和 TLS 证书规则来源。

nDPI 的识别链路比 Natflow MVP 更宽：同一应用可能由 DPI、Host/SNI/QNAME、IP 网段、TLS 证书、缓存和 master/app 复合协议共同确认。Natflow 初期只实现其中两类：

- **内置 detector app/proto**：把 nDPI 中有明确小状态机或固定结构的 detector 改写成 Natflow 自有有界 detector，输出 Natflow 自有 `proto_id`/`detector_id`。
- **域名规则 app**：把 nDPI `host_match[]` 中可见域名后缀整理成用户态规则包，通过 HTTP Host、TLS SNI、QUIC SNI 或 DNS QNAME 匹配到用户定义 `app_id`。

以下能力不直接进入 M1：nDPI IP 网段识别、TLS 证书主体匹配、跨流 LRU/cache 推断、DNS-to-flow 关联、STUN/媒体流复杂子分类、HASSH/JA3/JA4、品牌私有游戏 detector。它们只能在 M2/M4 按单项设计进入。

#### 7.5.1 清单字段与落地规则

所有清单项必须能落到以下字段，不能只写展示名称：

| 字段 | 含义 |
| --- | --- |
| `natflow_id` | Natflow 推荐 `proto_id` 或规则包 `app_id`；内核只固化 `proto_id`/`detector_id`，`app_id` 仍允许用户态重映射 |
| `ndpi_ref` | nDPI 参考协议名，例如 `NDPI_PROTOCOL_WIREGUARD`；只作来源说明，不作 UAPI 数值 |
| `source` | `detector`、`domain`、`cert`、`ip`、`cache` 或 `external`；M1 只允许 `detector` 和 `domain` |
| `stage` | `M1a`/`M1b`/`M1c`/`M1d`/`M2`/`M4` |
| `min_evidence` | 3-4 个包内能确认的最小字段、magic、方向阶段或后缀规则 |
| `policy_eligible` | `audit-only`、`shadow-policy` 或 `policy-after-shadow`；M1 统一为 `audit-only` |
| `corpus` | 合并前必须提供的正样本、相似负样本、端口伪装、畸形长度、重传/gap 和 IPv4/IPv6 样本 |

默认规则：内核不内置品牌名称、中文名、长域名列表或 nDPI 表；默认规则包由用户态生成并事务提交。nDPI 中没有来源的应用可以进入“外部规则包候选”，但不得列入默认支持，直到补齐域名样本、来源、冲突分析和 corpus。

#### 7.5.2 内置协议 detector 清单

M1 拆成小阶段。M1a/M1b 先让 gate、事件和域名类识别闭环；M1c 只加入 3 个高确定性非 HTTP/TLS/QUIC detector；M1d 再加入 STUN/BitTorrent 这类 nDPI 实现中已经出现缓存、复合协议或多形态判断的 detector。

| stage | Natflow 输出 | nDPI 参考 | source | 最小确认证据 | 包预算 | 策略 |
| --- | --- | --- | --- | --- | --- | --- |
| M1b | `proto_id=HTTP` | `NDPI_PROTOCOL_HTTP` | existing parser + detector | original 方向 request line + Host header | 4+0 | audit-only |
| M1b | `proto_id=TLS` | `NDPI_PROTOCOL_TLS` | existing parser + detector | TLS record、ClientHello、SNI/outer SNI extension | 4+0 | audit-only |
| M1b | `proto_id=QUIC` | `NDPI_PROTOCOL_QUIC` | existing parser + detector | QUIC v1 long header Initial、可解密 CRYPTO ClientHello、SNI | 4+0 UDP | audit-only |
| M1c | `proto_id=DNS` | `NDPI_PROTOCOL_DNS` | `dns.c` detector | DNS header、query/answer count、QNAME 边界、type/class；QNAME 可进 domain rule | 1+1 | audit-only |
| M1c | `proto_id=SSH` | `NDPI_PROTOCOL_SSH` | `ssh.c` detector subset | `SSH-` version exchange；M1 不做 HASSH、cipher list 或 extra packet fingerprint | 2+2 | audit-only |
| M1c | `proto_id=WIREGUARD` | `NDPI_PROTOCOL_WIREGUARD` | `wireguard.c` detector | UDP message type、reserved-zero、固定握手长度 148/92 或 204/100、反向 receiver index | 3+1 | audit-only |
| M1d | `proto_id=STUN` | `NDPI_PROTOCOL_STUN` | `stun.c` detector subset | STUN/TURN method、20 字节 header、magic cookie、attribute 长度边界 | 2+2 | audit-only |
| M1d | `app_id=BITTORRENT` 或 `proto_id=BT_DHT` | `NDPI_PROTOCOL_BITTORRENT` | `bittorrent.c` detector subset | TCP `BitTorrent protocol` handshake、uTP v1 header、DHT transaction 结构；M1d 不做跨流 LRU 端口推断 | 5+0 | audit-only |
| M2a | `proto_id=MQTT` | `NDPI_PROTOCOL_MQTT` | `mqtt.c` detector | CONNECT packet type、remaining length、protocol name/version、合法 flags | 4+1 | shadow-policy |
| M2a | `proto_id=MYSQL` | `NDPI_PROTOCOL_MYSQL` | `mysql.c` detector | server-first handshake、protocol version、capability/auth plugin 边界 | 1+2 | shadow-policy |
| M2a | `proto_id=POSTGRESQL` | `NDPI_PROTOCOL_POSTGRES` | `postgres.c` detector | startup/SSLRequest/GSSRequest/cancel request 或 server response stage | 2+2 | shadow-policy |
| M2a | `proto_id=SMB` | `NDPI_PROTOCOL_SMBV1`/`SMBV23` | `smb.c` detector | NetBIOS length + SMB1 `0xff SMB` 或 SMB2/3 `0xfe SMB` magic | 2+2 | shadow-policy |
| M2b | `proto_id=FTP` | `NDPI_PROTOCOL_FTP_CONTROL` | `ftp_control.c` detector | server banner、3 位 response code、USER/PASS/AUTH 等命令阶段 | 3+3 | shadow-policy |
| M2b | `proto_id=SMTP` | `NDPI_PROTOCOL_MAIL_SMTP` | `mail_smtp.c` detector | 220 banner、EHLO/HELO/MAIL/RCPT/response code | 4+4 | shadow-policy |
| M2b | `proto_id=POP3` | `NDPI_PROTOCOL_MAIL_POP` | `mail_pop.c` detector | `+OK` banner、USER/PASS/STAT/CAPA 等命令 | 4+4 | shadow-policy |
| M2b | `proto_id=IMAP` | `NDPI_PROTOCOL_MAIL_IMAP` | `mail_imap.c` detector | tagged command、`* OK` banner、CAPABILITY/LOGIN/STARTTLS | 4+4 | shadow-policy |
| M2b | `proto_id=SIP` | `NDPI_PROTOCOL_SIP` | `sip.c` detector | SIP request/status line、Via、Call-ID、CSeq 组合 | 4+4 | shadow-policy |
| M2b | `proto_id=RTSP` | `NDPI_PROTOCOL_RTSP` | `rtsp.c` detector | `RTSP/1.0` request/status line、CSeq、RTSP URL/header | 4+4 | shadow-policy |
| M2b | `proto_id=RESP` | `NDPI_PROTOCOL_RESP` | `resp.c` detector | RESP frame 边界、命令/响应 token；作为 Redis wire protocol 输出 | 4+4 | shadow-policy |
| M2b | `proto_id=RDP` | `NDPI_PROTOCOL_RDP` | `rdp.c`/`tls.c` detector | TPKT/X.224 connection request/confirm，必要时 TLS/RDP 组合 | 4+4 | shadow-policy |

M4/专项 detector：`NDPI_PROTOCOL_OPENVPN`、`SOFTETHER`、`KERBEROS`、`RTP`/`RTCP`、`TELEGRAM`/`WHATSAPP` 私有握手、`DISCORD`/`STEAM`/`EPICGAMES`/`RIOTGAMES`/`TENCENTGAMES` 私有 UDP/TCP、`CLOUDFLARE_WARP`、Tailscale、Shadowsocks/V2Ray/Trojan/VLESS 等代理/VPN。进入条件是单项设计，不得用端口/IP 或 nDPI 的缓存推断直接产生可阻断 `app_id`。

#### 7.5.3 默认域名规则包清单

以下 `app_id` 是 Natflow 推荐默认规则包编号，不是内核 ABI；部署可以重映射，但事件必须输出实际 `app_id`、`rule_id` 和 `ruleset_generation`。`source=domain` 表示可从 nDPI `host_match[]` 或生成域名表提取初始后缀；`source=external` 表示当前 nDPI 工作区没有对应内置依据，必须由维护者补样本后才能进入默认规则包。

| app_id | 名称 | nDPI 参考 | source | 初始后缀/证据来源 | 类别 | 策略资格 |
| --- | --- | --- | --- | --- | --- | --- |
| 10001 | Google | `NDPI_PROTOCOL_GOOGLE` | domain | `google*`、`gvt1.com`、`gvt2.com`、`gstatic/connectivitycheck` 等 nDPI 条目 | platform | policy-after-shadow |
| 10002 | YouTube | `NDPI_PROTOCOL_YOUTUBE` | domain | `youtube.`, `youtu.be`, `googlevideo.com`, `ytimg.com`, `youtubei.googleapis.com` | video | policy-after-shadow |
| 10003 | Netflix | `NDPI_PROTOCOL_NETFLIX` | domain | `netflix.com`, `nflxext.com`, `nflximg.*`, `nflxvideo.net`, `fast.com`; TLS `O=Netflix` 只进 M4 | video | policy-after-shadow |
| 10004 | TikTok | `NDPI_PROTOCOL_TIKTOK` | domain | `tiktokcdn.com`, `tiktokv.*`, `byteoversea.com`, `ibyteimg.com`, `musical.ly` | short-video | policy-after-shadow |
| 10005 | iQIYI | `NDPI_PROTOCOL_IQIYI` | domain | `iqiyi.com`, `iqiyipic.com`, `qy.net`, `iq.com`, `pps.tv`; UDP detector 另行评审 | video | policy-after-shadow |
| 10006 | Tencent Video | `NDPI_PROTOCOL_TENCENTVIDEO` | domain | `v.qq.com` 等 nDPI 条目；更多腾讯视频后缀需外部补齐 | video | shadow-policy |
| 10007 | WeChat | `NDPI_PROTOCOL_WECHAT` | domain | `wechat.com`, `wechatapp.com`, `weixin.`, `wx.`, `mmsns.qpic.cn` | chat | policy-after-shadow |
| 10008 | QQ | `NDPI_PROTOCOL_QQ` | domain | `qq.com`, `gtimg.com` | chat | policy-after-shadow |
| 10009 | WhatsApp | `NDPI_PROTOCOL_WHATSAPP` | domain | `whatsapp.`, `whatsapp.net`, `chat-e2ee-mini.facebook.com`; 私有 TCP detector 不进 M1 | chat | policy-after-shadow |
| 10010 | Telegram | `NDPI_PROTOCOL_TELEGRAM` | domain | `web.telegram.org`, `t.me`, `telegram.me`, `tdesktop.com`, `cdn-telegram.org`; 私有 detector 另行评审 | chat | policy-after-shadow |
| 10011 | Discord | `NDPI_PROTOCOL_DISCORD` | domain | `discord.com`, `discord.gg`, `discordapp.com`, `discord.media`; UDP detector 另行评审 | collaboration | policy-after-shadow |
| 10012 | Facebook | `NDPI_PROTOCOL_FACEBOOK` | domain | `facebook.com`, `facebook.net`, `fbcdn.net`, `fbsbx.com`, `fb.com` | social | policy-after-shadow |
| 10013 | Instagram | `NDPI_PROTOCOL_INSTAGRAM` | domain | `instagram.`, `cdninstagram.com`, `igcdn-photos-`, `instagramimages-`, `instagramstatic-` | social | policy-after-shadow |
| 10014 | X/Twitter | `NDPI_PROTOCOL_TWITTER` | domain | `twitter.`, `twimg.com`, `t.co`, `x.com`, `twttr.com` | social | policy-after-shadow |
| 10015 | Microsoft | `NDPI_PROTOCOL_MICROSOFT` | domain | `microsoft.com`, `msocdn.com`, `msedge.net`, `msftstatic.com`, `msn.com` | platform | shadow-policy |
| 10016 | Microsoft 365/Teams | `NDPI_PROTOCOL_MICROSOFT_365`/`MSTEAMS` | domain | Teams/Office 相关 nDPI 条目；`MSTEAMS_CALL`/STUN 子分类和证书规则不进 M1 | collaboration | policy-after-shadow |
| 10017 | OneDrive | `NDPI_PROTOCOL_MS_ONE_DRIVE` | domain | `admin.onedrive.com`, `oneclient.sfx.ms`, `sharepoint.com`, `storage.live.com`, `svc.ms` | cloud | policy-after-shadow |
| 10018 | Apple | `NDPI_PROTOCOL_APPLE` | domain | `apple.com`, `mzstatic.com`, `aaplimg.com`, `cdn-apple.com`, `apple-cloudkit.com` | platform | shadow-policy |
| 10019 | Apple iCloud | `NDPI_PROTOCOL_APPLE_ICLOUD` | domain | `icloud.com`, `me.com`, `icloud-content.com` | cloud | policy-after-shadow |
| 10020 | Amazon | `NDPI_PROTOCOL_AMAZON` | domain | `amazon.`, `amazon.com`, `media-amazon.com`, `images-amazon.com` | shopping/platform | policy-after-shadow |
| 10021 | Amazon AWS | `NDPI_PROTOCOL_AMAZON_AWS` | domain | `amazonaws.com`, `aws.a2z.com`; AWS service split 只进后续规则包 | cloud/infra | audit-or-qos |
| 10022 | Alibaba/Alipay | `NDPI_PROTOCOL_ALIBABA` | domain | `alibaba.com`, `alicdn.com`, `aliyuncs.com`, `alipay.com`, `alipayobjects.com` | shopping/payment/cloud | policy-after-shadow |
| 10023 | Taobao/Tmall | `NDPI_PROTOCOL_TAOBAO` | domain | `taobao.com`, `tmall.com` | shopping | policy-after-shadow |
| 10024 | Steam | `NDPI_PROTOCOL_STEAM` | domain | `steampowered.com`, `steamcommunity.com`, `steamcontent.com`, `steamstatic.com`; UDP detector 另行评审 | game | policy-after-shadow |
| 10025 | Epic Games | `NDPI_PROTOCOL_EPICGAMES` | domain | `epicgames.com`, `epicgames.net`, `unrealengine.com`, `fortnite.com` | game | policy-after-shadow |
| 10026 | Riot Games | `NDPI_PROTOCOL_RIOTGAMES` | domain | `riotgames.com`, `pvp.net`, `riotcdn.net`; Riot TLS cert 只进 M4 | game | policy-after-shadow |
| 10027 | PlayStation Network | `NDPI_PROTOCOL_PLAYSTATION` | domain | `playstation.net`, `playstation.com`, `sonyentertainmentnetwork.com`, `wbagora.com`, `wbplay.com` | game | policy-after-shadow |
| 10028 | Xbox Live | `NDPI_PROTOCOL_XBOX` | domain | `xbox.com`, `xboxlive.com`, `xboxservices.com`, `xboxab.com` | game | policy-after-shadow |
| 10029 | Cloudflare | `NDPI_PROTOCOL_CLOUDFLARE` | domain | `cloudflare.com`, `cdnjs.cloudflare.com`, `cf-ipfs.com`; IP 网段和 TLS `O=Cloudflare` 只进 M4 | infra/cdn | audit-or-qos |

外部规则包候选：Baidu、Bing 细分、Douyin、Kuaishou、Bilibili、Youku、JD.com、Meituan、Tencent Games 默认域名、Fastly 等当前 nDPI 工作区没有足够可直接引用的 `host_match` 条目或在本设计未完成抽取。它们可以由用户态规则包支持，但默认清单中必须标注 `source=external`、样本来源、后缀数量和冲突域名；没有这些材料前不能宣称内置支持。

IP-only/共享基础设施候选：`NDPI_PROTOCOL_AKAMAI`、Cloudflare IP 网段、OneDrive IP 网段、AWS IP 网段等来自 nDPI 生成 IP 表，不进入 M1 默认域名规则包。它们只能在 M4 或用户态规则包中作为审计/QoS hint，不能单独产生可阻断 `app_id`。
...
- 增加最小全局 flow context registry、terminal state、过期 drain、enable/disable 状态机和 reason counter。
- 实现 `/dev/natflow_dpi_ctl` 的空 ruleset 事务、`enable=0|1`、状态 read，以及 `/dev/natflow_dpi_queue` 的固定 header 事件。
- 此阶段可以只输出 `NO_DETECTOR`/`DPI_DISABLED`/`CACHE_FULL` 等 terminal event，不宣称应用识别。
- `nf->status` 非原子仍按已接受限制处理：DPI 不迁移整字全部 writer，也不增加 path 侧 repair，但必须有 lost-owner/early-fastpath 计数和并发压测。

### M1b：共享 HTTP/TLS/QUIC 与 domain rules

- 抽出 HTTP Host、TLS SNI、QUIC v1 Initial SNI 的共享 parser API 和 hostname normalize/validate。
- legacy URL logger 改为消费共享 features，保持现有设备/CSV/sysctl ABI。
- 实现 app/domain snapshot、exact/suffix matcher、transactional commit 和 `DIRECT_SUFFIX`/`DIRECT_EXACT` 事件。
- 在 M1b 退出前删除 HTTP/TLS/QUIC 的临时重复 parser；URL logger、Host ACL 和 DPI 必须消费同一次 parser 结果。
- 默认关闭、unknown/error fail-open；仍不执行 app ACL/QoS。

### M1c：首批非 HTTP/TLS/QUIC protocol-only detector

- 按 7.5.2 实现 DNS、SSH、WireGuard 三个 detector，全部先输出 `DIRECT_PROTOCOL` 或 audit-only `DIRECT_APP`，不进入阻断/QoS。
- DNS 只使用本 DNS flow 的 QNAME 做审计或 domain rule 命中，不给后续连接做强关联。
- SSH 只做 `SSH-` version exchange/protocol-only，不做 HASSH、cipher list、MD5 fingerprint 或 extra packet fingerprint。
- WireGuard 使用固定 message type、reserved-zero、握手长度和方向阶段确认；不因 UDP 51820 单独命中。

### M1d：第二批 A 级 detector 和 shadow 准备

- 按 7.5.2 实现 STUN/TURN protocol-only 和 BitTorrent handshake/uTP/DHT 子集。
- STUN 不做 WhatsApp/Google/Teams/Telegram VoIP 子分类，不使用 LRU/cache 推断媒体流。
- BitTorrent 不做跨流 LRU 端口扩展，不因域名或端口单独命中；只接受明确握手、uTP v1 或 DHT 结构。
- 补齐 detector coverage、protocol-only rate、app hit、unknown reason、false positive sample、cache/ring loss 和 p99 hook latency 统计，为 M2 shadow 做准备。
...
- 按数据逐步加入 B 级 detector，例如 FTP/SMTP/POP3/IMAP/SIP/RTSP/MQTT/RESP/MySQL/PostgreSQL/RDP/SMB。每个新 detector 必须带 corpus 和可关闭开关。
