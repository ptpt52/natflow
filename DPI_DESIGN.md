...
1. MVP 只支持 HTTP/1 Host、TLS ClientHello SNI、QUIC v1 Initial SNI 到 `app_id` 的域名映射，以及独立的被动审计事件。
...
7. 规则以不可变 snapshot 发布；控制面事务提交，数据面 RCU 读取，active context 绑定实际使用的 ruleset generation，并在 terminal event 中输出；flow 常驻结果只保留 `app_id`。
...
| `app_id` | 由当前 ruleset 对元数据映射出的用户定义应用 | `app_id=1001` |
...
- DPI 状态机、`rule_id`、`ruleset_generation`、`category_id`、`l7_proto`、`evidence`、`confidence`、`reason`、`inspected_bytes`、payload packet 计数和 policy action 都属于 active context 或 terminal event 数据，不常驻 conntrack。需要审计时在 terminal 当场序列化到 `/dev/natflow_dpi_queue`；event ring 满时这些诊断信息可以丢失，只保留 flow 上的 `app_id`。
...
- 扩大 `natflow_t` 是 M0/M1 的设计前置门槛，必须先在真实 NATCAP 组合上验证 `nat_key_t.len`、`natflow_off`、`NATCAP_MAX_OFF` 和现有脆弱 krealloc 假设。v3 不提供长期 result side-table fallback；验证不通过的 build 必须拒绝 `CONFIG_NATFLOW_DPI`，不能在实现后期静默换存储模型。
...
- DPI 运行时默认 `enable=0`。DPI enable 不改变 legacy `/proc/sys/urllogger_store/enable` 语义：该 sysctl 仍同时控制 URL CSV 记录和 Host ACL 处理；`enable=0` 时不执行 Host ACL，即使 DPI 已启用。parser activation 是 DPI consumer 与 legacy URL/HostACL consumer 的 OR。
...
- terminal event 可区分 `app_id=0`、protocol-only、matched 和 error，并带 generation/reason；flow 常驻结果只保留 `app_id`。
