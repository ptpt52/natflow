...
- 当前 URL logger hook 的生命周期由 `natflow_l7_init()/exit()` 触发，URL hook ops 和内核 hook 签名兼容包装已由 `natflow_l7.c` 持有：默认注册 IPv4、IPv6 和 bridge `FORWARD` hook，优先级 `NF_IP_PRI_FILTER + 5`；可选 `CONFIG_NATFLOW_URLLOGGER_LOCAL_IN` 改为 IPv4 `LOCAL_IN`。底层数据面仍委托 `natflow_urllogger.c` 中的 legacy URL consumer。
...
4. `natflow_l7` 已成为 URL hook lifecycle owner，并持有 URL hook ops 与签名兼容包装；但还没有共享 context pool 或 crypto capability。
...
- DPI 在 selected flow arm 成功后设置 `NF_FF_DPI_USE`，terminal 后清除。
...
- writer 在 matched 结果时先写 `app_id`，再写 terminal state，最后清 `NF_FF_DPI_USE`。unknown/error/disabled 保持 `app_id=0`。
...
- `NF_FF_DPI_USE` 正常阻止 fast path 提前接管 selected flow。
