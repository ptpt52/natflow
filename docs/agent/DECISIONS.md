...
当前 L7 入口只准入 conntrack original direction，且 `NF_FF_L7_DPI_PACKET_DONE` 是连接级单一终态。直接放开 reply 会让任一方向的首包过早关闭另一个方向，或者让没有 reply payload 的单向流量长期阻塞 fast path。不同协议实际需要 original-only、server-first、任一方向或双向关联四种不同语义。
...
- 只有等待方向、跨包 prefix 或关联阶段的 detector 才分配 bounded context。常驻 conntrack 分类结果仍只有 `app_id`，方向、预算、阶段和证据只进入 active context 或 terminal event。
- context 等待期间使用 `NF_FF_DPI_USE` 阻止 fast path；任一 detector 命中，或所有 active detector 因明确不匹配、FIN/RST、packet/byte budget、4 秒 deadline 或资源失败终态后，连接级 DPI packet consumer 才终态。
...
- 初始预算为 original/reply 各 4 个 payload 包、4 秒 deadline。资源不足 fail-open；不通过配置变更枚举、drain 或清理 conntrack/context。
