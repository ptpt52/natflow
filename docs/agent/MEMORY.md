...
| `natflow_l7.c/.h` | L7 hook 生命周期骨架和共享 feature core；当前持有 URL hook ops、内核 hook 签名兼容包装和注册/注销流程，委托 legacy urllogger consumer，提供 packet view、host/URI normalize、feature 结构、HTTP Host parser、TLS ClientHello/SNI 搜索、QUIC Initial header/CRYPTO frame/SNI 搜索和 DNS QNAME parser；QUIC crypto context/cache 生命周期仍由 URL logger 持有。 |
