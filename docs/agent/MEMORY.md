...
| `natflow_dpi.c/.h` | DPI 控制/事件接口；DPI packet consumer 根据 active protocol mask 和方向预算运行 A/B 级静态 detector，使用 `natflow_t` 的 8 字节瞬态 context。DNS reply 需要 response header/question 结构证据。 |
...
- 2026-08-15 DPI 新目标架构已接受：`DPI_HARDCODED_STATE_MACHINE_DESIGN.md` 取代离线 DSL/compiler 方向。前四层完成 L3/L4、基础协议和结构化 feature，只有第五层手写 C 应用状态机到达终态才直接写固定 `app_id`；取消用户 domain/proto 规则和 app 重映射，保留 enable、统计和事件观测。迁移按固定 catalog、protocol detector 静态化、domain classifier、ruleset removal、compact automaton 分阶段进行。M1 已固定首批 18 个协议应用 ID/category 并增加 catalog 初始化校验，但尚未切换当前 ruleset 驱动的数据面；迁移完成前源码现有 ruleset 行为仍是事实基线。
- 2026-07-18 protocol-only detector 收紧为直接协议证据：DNS 必须解析为 TCP/UDP 53 标准 query，SSH 必须在 TCP 任一方向匹配 banner，WireGuard 必须通过 UDP message type/reserved bytes/长度校验；TCP 22 和 UDP 51820 不再直接分类。数据面通过 active protocol mask 跳过未配置 detector。
