...
- `/dev/natflow_userinfo_queue` 只允许一个 reader，open 后进入 RUNNING 并开始接收认证相关事件；没有 reader 时事件直接不入队，reader 关闭会清空未读事件。`read()` 空队列返回 0，不挂起；`poll()` 在有事件时返回 readable。输出 v2 固定头二进制 `natflow_userinfo_event_hdr`，采用统一 event header 编排，字段语义与 `/dev/userinfo_ctl` 文本快照一致。
