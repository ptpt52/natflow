...
| `natflow_l7.c/.h` | L7 hook 生命周期骨架和共享 feature core；当前负责集中触发 legacy URL hook 注册/注销，提供 packet view、host/URI normalize、feature 结构、HTTP Host parser 和 TLS ClientHello/SNI 搜索，QUIC Initial 解密/拼接仍在迁移中。 |
