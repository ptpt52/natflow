...
| `natflow_dpi.c/.h` | DPI 控制/事件接口；当前提供默认关闭的 `/dev/natflow_dpi_ctl`、domain exact/suffix ruleset、DNS QNAME domain 分类、DNS/SSH/WireGuard/STUN/TURN/BitTorrent protocol-only ruleset、L7 packet-view consumer、SSH banner/BitTorrent/STUN/TURN 有界 payload detector、match event 队列、source/reason counters、`events_clear` 测试辅助命令和 `app_id` 写入。 |
