...
- auth 的规则/数量/bypass/magic 与 QoS 的规则/数量/classid/速率分别为不可变 RCU 快照，写端按各自 mutex 克隆发布并等待 grace period 后回收。auth 的 `disabled`、redirect、timeout、HTTPS/WeChat 开关及未知命令改走独立 `auth_scalar_lock`，不分配/发布快照，也不等待 auth grace period；auth seq reader 按 `auth_conf_lock` → `auth_scalar_lock` 同时持锁，写端经 input mutex 后只持其中一把。QoS seq 仍只持配置 mutex。Netfilter hook 使用外层 RCU，rx/tx token 入口显式取 RCU；数据面不取 mutex。auth 的 `update_magic` 仍手动触发；QoS bucket 不随快照复制，速率作为参数传入原 bucket 锁保护的更新，保留 qos_id 槽位复用和 clear 不复位余额的语义。
...
- plain vline 的 NOARP link-local NS→NA 模拟回复先验证 target 再改长度，用 skb_put/trim 保持 tail/len 一致，更新 IPv6 payload_len 和 Target LL option；DAD 回复 ff02::1 且 solicited=0，准备失败时 drop。
- ND relay 选项长度使用 unsigned int 计算 `len * 8`，通过 IPv6 payload_len 内的剩余长度遍历，不能用 u8 保存字节长度，也不能将 skb 尾部 padding 当成选项或 checksum 输入。
...
- `DPI_DESIGN.md` Draft v7 把 P2 统一为 `natflow_l7` core。`app_id` 是唯一常驻分类结果；`natflow_t` 尾部另有 8 字节有效瞬态 DPI 状态，仅在 `NF_FF_DPI_USE` 时保存双向预算和 16 位 automaton 编码。编码存放在自然对齐的 32 位 `dpi_automaton` 低半部，高 16 位保留为零；`bit15=0` 时低 8 位是 discovery machine-class mask，`bit15=1` 时 `bits14..8` 是 claimed machine、`bits7..0` 是 state。RDP、SOCKS、WhatsApp 是当前 claimed machine；context 不保存证据字符串、规则详情或指针。
- `natflow_t` 的 DPI 尾部布局为 4 字节 byte counters、2 字节 packet counters、2 字节显式 `layout_pad` 和 4 字节 `dpi_automaton`，共 12 字节。扩宽 automaton 复用了原有尾部 padding，`sizeof(natflow_t)` 和 conntrack ext 对齐分配长度仍为 96 字节；编译期断言同时约束字段顺序、大小和 32 位自然对齐。
...
- 2026-08-15 DPI M2/M5 B 级原生协议机器：FTP、SMTP、POP3、IMAP、SIP、RTSP、MQTT、RESP、MySQL、PostgreSQL、RDP、SMB 按文本、数据库、二进制三组占用剩余 3 个 discovery machine-class bit，不扩大 conntrack context；MySQL 只认 reply v10 greeting，MQTT/RESP/PostgreSQL 只认 original 结构首包，歧义 `USER`、端口和普通 banner 不分类。M5 后 RDP 不再由单个 TPKT/X.224 首包终态：16 位 automaton 编码以对齐 32 位 word 的 `cmpxchg()` 单调 OR original Connection Request 与 reply confirm，两个事实任意顺序汇合才提交，claimed 后只运行 RDP machine。TCP sequence corpus 覆盖顺序、反序、同时发送、重复、双向预算、transport end 和 malformed confirm；自动 corpus 总数为 93，新增样本真机 IPv4/IPv6 尚待完成。
...
- 认证/Host ACL 的独立 TCP 回复用 `skb_copy_expand()` 后仍继承原包 GSO 和头偏移；重建头部及软件 checksum 后，发送前必须调用 `natflow_tcp_reply_prepare()` 重设 transport header、清 GSO 并标记 `CHECKSUM_NONE`，避免短回复误入分段路径。
