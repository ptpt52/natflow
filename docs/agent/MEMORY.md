...
| `natflow_l7.c/.h` | L7 hook 生命周期骨架和共享 feature core；当前持有 URL/DPI shared hook ops、内核 hook 签名兼容包装、PPPoE normalize/restore、基础 conntrack 过滤、TCP HTTP/TLS producer、QUIC UDP producer、TCP TLS SNI cache、QUIC cache/crypto、DPI packet consumer 调度和注册/注销流程，向 legacy urllogger consumer 或 DPI-only host classifier 传入 host view，并提供 host/URI normalize、feature/host view 结构、HTTP Host parser、TLS ClientHello/SNI 搜索、QUIC Initial header/CRYPTO frame/SNI 搜索和 DNS QNAME parser。L7 packet view 由 producer 填充 L3/L4/payload 指针、payload 长度和已线性化的有界 payload 窗口。 |
| `natflow_dpi.c/.h` | DPI 控制/事件接口；当前提供默认关闭的 `/dev/natflow_dpi_ctl`、domain exact/suffix ruleset、DNS QNAME domain 分类、DNS/SSH/WireGuard/STUN/TURN/BitTorrent protocol-only ruleset、L7 packet-view consumer、SSH banner/BitTorrent/STUN/TURN 有界 payload detector、match event 队列、source/reason counters、`events_clear` 测试辅助命令和 `app_id` 写入。DPI packet consumer 只消费 L7 packet view 的 L4/payload 字段，不再自行重解析 IPv4/IPv6 skb。 |
| `natflow_path.c/.h` | fast path、路由学习、vline/relay、设备 notifier、硬件 offload。 |
| `natflow_user.c/.h` | fakeuser、认证、QoS、用户信息控制设备和 `/dev/natflow_userinfo_queue` 阻塞式二进制认证事件流。 |
| `natflow_urllogger.c/.h` | Legacy URL consumer；通过 `natflow_urllogger_consume_host_view()` 消费 L7 host view，处理 URL record、Host ACL、DPI classify 和 ACL 回复策略，保留 HTTP Host/URI、TLS/QUIC SNI 的 URL 记录、URL store、Host ACL、302/RST 动作和 sysctl 资源。`/dev/natflow_urllogger_queue` 只允许一个 reader；没有 reader 时 URL/SNI record 在 ACL/DPI 处理后直接丢弃，不缓存到 URL store。 |
...
- `/dev/natflow_dpi_queue` 只允许一个 reader；没有 reader 时 match event 直接丢弃，不分配、不缓存，也不增加 `events_lost`；reader 打开会清空残留事件，关闭会清空未读事件，有 reader 期间队列上限仍为 1024；事件 `timestamp` 是 uptime 秒数，与 URL logger 一致；事件 ABI 为 v2 固定头，采用统一 event header 编排，包含 original tuple 的 `family/l4proto/tuple_dir/sport/dport/sip/dip`。
- `/dev/natflow_urllogger_queue` 只允许一个 reader；没有 reader 时 URL/SNI record 直接丢弃，不缓存、不增加 `urllogger_store/memsize` 或 `count`；reader 打开会清空残留记录，关闭会清空未读记录。`read()` 为空返回 0，输出 v2 版本化二进制 record，固定头是 `natflow_urllogger_event_hdr`，采用统一 event header 编排，payload 是不带结尾 NUL 的 `host + uri`，长期采集应保持 fd 并用带超时的 `poll()`/`select()`/`epoll`，因为 `timestamp_freq` 仍是读出前的老化窗口。
- `/dev/natflow_userinfo_queue` 只允许一个 reader，open 后进入 RUNNING 并开始接收认证相关事件；没有 reader 时事件直接不入队，reader 关闭会清空未读事件。`read()` 是阻塞等待，输出 v2 固定头二进制 `natflow_userinfo_event_hdr`，采用统一 event header 编排，字段语义与 `/dev/userinfo_ctl` 文本快照一致。
