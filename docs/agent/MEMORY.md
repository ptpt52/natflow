...
| `natflow_dpi.c/.h` | DPI 控制/事件接口；DPI enabled 后 packet consumer 根据 L4、方向和 discovery machine-class mask 运行 18 个固定原生协议机器，使用 `natflow_t` 的 8 字节瞬态 context。单包机器命中直接提交固定 app/category；RDP compact automaton 要求双向 X.224 request/confirm，DNS reply 要求 response header/question 结构证据。 |
...
- `DPI_DESIGN.md` Draft v7 把 P2 统一为 `natflow_l7` core。`app_id` 是唯一常驻分类结果；`natflow_t` 尾部另有 8 字节瞬态 DPI context，仅在 `NF_FF_DPI_USE` 时保存双向预算和 16 位 `dpi_automaton`。`bit15=0` 时低 8 位是 discovery machine-class mask；`bit15=1` 时 `bits14..8` 是 claimed machine、`bits7..0` 是 state。RDP 是首台 claimed machine；context 不保存证据字符串、规则详情或指针。
...
- 当前源码已把 bit 19 收敛为 `NF_FF_L7_USE` shared L7 fast-path pause 位，`NF_FF_DPI_USE_BIT=21` 标记 `natflow_t` 内瞬态 DPI context 并纳入 `NF_FF_BUSY_USE`，三个 URL/DPI consumer done bit 独立记录终态。L7 core 持有共享 FORWARD hook、packet/host view、HTTP Host、TLS/QUIC SNI 和 DNS parser；URL consumer、DPI domain 和 DPI packet consumer 消费共享结果。DPI enable 直接发布静态 host 和固定原生协议 consumer，不再有 domain/proto ruleset。DPI packet consumer 直接消费 L7 producer 提供的 L4/payload 指针和有界 `payload_linear_len`，按固定原生 machine step 识别 18 个协议；源码没有 detector struct、metadata 数组或通用 detector dispatcher。HTTP/TLS/QUIC host 分类、DNS QNAME 意图和原生协议机器均不依赖 `/proc/sys/urllogger_store/enable`。维护者接受 `nf->status` 非原子 writer 风险，不做 path 侧 repair。
