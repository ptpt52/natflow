...
- `DPI_DESIGN.md` Draft v7 把 P2 统一为 `natflow_l7` core。`app_id` 是唯一常驻分类结果；`natflow_t` 尾部另有 8 字节瞬态 DPI context，仅在 `NF_FF_DPI_USE` 时保存双向预算和 16 位 `dpi_automaton`，其低 8 位当前保持 discovery detector mask，高 8 位为 claimed machine/state 保留；context 不保存证据字符串、规则详情或指针。
