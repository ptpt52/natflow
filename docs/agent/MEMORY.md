...
| `natflow_user.c/.h` | fakeuser、认证、QoS、用户信息控制设备和 `/dev/natflow_userinfo_event_queue` 阻塞式二进制认证事件流。 |
...
- `/dev/natflow_dpi_queue` 只允许一个 reader；没有 reader 时 match event 直接丢弃，不分配、不缓存，也不增加 `events_lost`；reader 打开会清空残留事件，关闭会清空未读事件，有 reader 期间队列上限仍为 1024。
...
- `/dev/natflow_userinfo_event_queue` 只允许一个 reader，open 后进入 RUNNING 并开始接收认证相关事件；没有 reader 时事件直接不入队，reader 关闭会清空未读事件。`read()` 是阻塞等待，输出固定头二进制 `natflow_userinfo_event_hdr`，字段语义与 `/dev/userinfo_ctl` 文本快照一致。
