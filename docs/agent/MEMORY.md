...
| `natflow_dpi.c/.h` | DPI 控制/事件接口；DPI packet consumer 根据 active protocol mask 运行静态 detector，未命中时在 `natflow_t` 的 8 字节瞬态 context 中记录双向 packet/byte counter 和 detector mask，并设置 `NF_FF_DPI_USE`。当前 reply 仍被公共入口过滤。 |
...
- 2026-07-18 M1e bounded context 已进入 `natflow_t` 尾部：双向 byte counter 4 字节、packet counter 2 字节、detector mask 1 字节和 reserved 1 字节。未命中不再立即写 packet done；命中、已有 app、FIN/RST 或所有 detector 双向预算耗尽时清 context 并终态。reply 仍被入口过滤。
