...
| `natflow_urllogger.c/.h` | Legacy URL consumer；通过 `natflow_urllogger_consume_host_view()` 消费 L7 host view，处理 URL record、Host ACL、DPI classify 和 ACL 回复策略，保留 HTTP Host/URI、TLS/QUIC SNI 的 URL 记录、URL store、Host ACL、302/RST 动作和 sysctl 资源。`/dev/urllogger_queue` 只允许一个 reader；没有 reader 时 URL/SNI record 在 ACL/DPI 处理后直接丢弃，不缓存到 URL store。 |
...
- `/dev/urllogger_queue` 只允许一个 reader；没有 reader 时 URL/SNI record 直接丢弃，不缓存、不增加 `urllogger_store/memsize` 或 `count`；reader 打开会清空残留记录，关闭会清空未读记录。`read()` 为空返回 0，长期采集应保持 fd 并用带超时的 `poll()`/`select()`/`epoll`，因为 `timestamp_freq` 仍是读出前的老化窗口。
