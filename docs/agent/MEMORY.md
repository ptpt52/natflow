...
- `/dev/natflow_dpi_queue` 只允许一个 reader；没有 reader 时 match event 直接丢弃，不分配、不缓存，也不增加 `events_lost`；reader 打开会清空残留事件，关闭会清空未读事件，有 reader 期间队列上限仍为 1024；事件 `timestamp` 是 uptime 秒数，与 URL logger 一致。
- `/dev/natflow_urllogger_queue` 只允许一个 reader；没有 reader 时 URL/SNI record 直接丢弃，不缓存、不增加 `urllogger_store/memsize` 或 `count`；reader 打开会清空残留记录，关闭会清空未读记录。`read()` 为空返回 0，输出版本化二进制 record，固定头是 `natflow_urllogger_event_hdr`，payload 是不带结尾 NUL 的 `host + uri`，长期采集应保持 fd 并用带超时的 `poll()`/`select()`/`epoll`，因为 `timestamp_freq` 仍是读出前的老化窗口。
- `/dev/natflow_userinfo_queue` 只允许一个 reader，open 后进入 RUNNING 并开始接收认证相关事件；没有 reader 时事件直接不入队，reader 关闭会清空未读事件。`read()` 是阻塞等待，输出固定头二进制 `natflow_userinfo_event_hdr`，字段语义与 `/dev/userinfo_ctl` 文本快照一致。
