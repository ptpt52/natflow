...
| `natflow_dpi.c/.h` | DPI 控制/事件接口；DPI enabled 后 packet consumer 根据固定 protocol mask、L4 和方向预算运行 A/B 级静态 detector，使用 `natflow_t` 的 8 字节瞬态 context。单包 detector 命中直接提交固定 app/category；RDP compact automaton 要求双向 X.224 request/confirm，DNS reply 要求 response header/question 结构证据。 |
...
- `DPI_DESIGN.md` Draft v7 把 P2 统一为 `natflow_l7` core。`app_id` 是唯一常驻分类结果；`natflow_t` 尾部另有 8 字节瞬态 DPI context，仅在 `NF_FF_DPI_USE` 时保存双向预算和 16 位 `dpi_automaton`。`bit15=0` 时低 8 位是 discovery detector mask；`bit15=1` 时 `bits14..8` 是 claimed machine、`bits7..0` 是 state。RDP 是首台 claimed machine；context 不保存证据字符串、规则详情或指针。
...
- 2026-08-15 DPI 新目标架构已接受：`DPI_HARDCODED_STATE_MACHINE_DESIGN.md` 取代离线 DSL/compiler 方向。前四层完成 L3/L4、基础协议和结构化 feature，只有第五层手写 C 应用状态机到达终态才直接写固定 `app_id`；取消用户 domain/proto 规则和 app 重映射，保留 enable、统计和事件观测。M1-M3 已固定 18 个协议 app、YouTube=`0x1001`、Netflix=`0x1002`、Telegram=`0x2001` 和 14 项静态 exact/label-boundary suffix 域名表；HTTP Host、TLS/QUIC SNI 可提交固定应用，DNS QNAME 只累计 `dns_app_intents` 而 DNS flow 仍提交 DNS。M4 已删除 domain/proto ruleset、pending/active RCU 对象、规则 parser/count 和规则 ctl ABI；ctl status 不再输出 generation/rules/domain_rules/txn_active，只保留 enable、catalog、counters 和 `events_clear`。所有 event 使用 catalog revision、固定 category 和 `rule_id=0`。
...
- 2026-08-15 detector corpus 框架：`tests/dpi/run-corpus.sh` 建立两个 network namespace，经 root namespace 的真实 FORWARD hook 注入每例独立的 TCP/UDP 连接；默认使用 IPv4，`--ipv6` 使用两个 IPv6 `/64`。`tools/natflow-dpi-traffic.c` 生成 original/reply payload，`tools/natflow-dpi-corpus.c` 先打开 queue，再按 original tuple 过滤并断言 v3 固定 catalog revision、app/category/rule/evidence。退出时恢复 enable 和对应 family forwarding，清理 iptables/ip6tables 与 namespace；最终 PASS 前核验状态均已恢复。它会清空事件统计，只用于隔离测试环境。
...
- 2026-08-15 DPI M2/M5 B 级 detector：FTP、SMTP、POP3、IMAP、SIP、RTSP、MQTT、RESP、MySQL、PostgreSQL、RDP、SMB 按文本、数据库、二进制三组占用剩余 3 个 discovery mask bit，不扩大 conntrack context；MySQL 只认 reply v10 greeting，MQTT/RESP/PostgreSQL 只认 original 结构首包，歧义 `USER`、端口和普通 banner 不分类。M5 后 RDP 不再由单个 TPKT/X.224 首包终态：16 位 automaton 用 `cmpxchg()` 单调 OR original Connection Request 与 reply Connection Confirm，两个事实任意顺序汇合才提交，claimed 后只运行 RDP machine。TCP sequence corpus 覆盖顺序、反序、同时发送、重复、双向预算、transport end 和 malformed confirm；自动 corpus 总数为 92，新增样本真机 IPv4/IPv6 尚待完成。
