...
- `DPI_DESIGN.md` Draft v4 把 P2 DPI 定位为有界 L7 detector 框架：MVP 不再只限 HTTP Host、TLS/QUIC SNI 域名映射，而是先建立 `NF_FF_DPI_USE`/`NF_FF_BUSY_USE` gate、统一 packet view、detector dispatcher、终态原因、事务规则、`proto_id`/`detector_id` 版本化事件和资源硬上限；7.5 节列出初始支持清单，M1 包含 HTTP/1、TLS、QUIC v1、DNS、STUN/TURN、SSH、BitTorrent、WireGuard，M2 再按 shadow 数据加入 FTP/SMTP/POP3/IMAP/SIP/RTSP/MQTT/Redis/MySQL/PostgreSQL/RDP/SMB 等，品牌应用主要通过 Host/SNI/QNAME 域名规则包识别；端口/IP/DNS 关联等弱证据不得单独阻断。维护者接受 `nf->status` 非原子 writer 可能导致 owner/QoS/route 位丢失的风险，不设计 path 侧 gate repair；legacy `/proc/sys/urllogger_store/enable` 仍同时控制 URL CSV 和 Host ACL；MVP flow result 只在 `natflow_t` 尾部常驻 `u32 app_id`，其他审计字段只进 terminal event；编码前仍必须确认 bit 21、NATCAP 扩展尾增、conntrack ext probe 和厂商 offload/new-flow 回慢路径契约。
