...
- `DPI_DESIGN.md` Draft v4 把 P2 DPI 定位为有界 L7 detector 框架：7.5 节已基于 `/home/ubuntu/nDPI` 源码把 nDPI DPI detector、host_match/生成域名表、IP/证书/cache/复合协议能力分开；Natflow UAPI 不复制 nDPI enum，内核只固化自有 `proto_id`/`detector_id`，品牌应用通过用户态 Host/SNI/QNAME 规则包映射 `app_id`。M1 已拆为 M1a gate/ABI、M1b HTTP/TLS/QUIC+domain、M1c DNS/SSH/WireGuard、M1d STUN/BitTorrent 子集；默认域名包只纳入有 nDPI 域名依据的条目，Xbox/PlayStation 可进默认域名包，TencentGames/Akamai/IP-only CDN 只能走专项 detector、外部规则或 M4 审计/QoS hint；M2 再按 shadow 数据加入 FTP/SMTP/POP3/IMAP/SIP/RTSP/MQTT/RESP/MySQL/PostgreSQL/RDP/SMB 等。端口/IP/DNS 关联等弱证据不得单独阻断。维护者接受 `nf->status` 非原子 writer 可能导致 owner/QoS/route 位丢失的风险，不设计 path 侧 gate repair，但必须保留 lost-owner/early-fastpath 计数和并发压测；legacy `/proc/sys/urllogger_store/enable` 仍同时控制 URL CSV 和 Host ACL；MVP flow result 只在 `natflow_t` 尾部常驻 `u32 app_id`，其他审计字段只进 terminal event；编码前仍必须确认 bit 21、NATCAP 扩展尾增、conntrack ext probe 和厂商 offload/new-flow 回慢路径契约。
