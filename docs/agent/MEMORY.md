...
| `natflow_dpi.c/.h` | DPI 控制/事件接口；DPI packet consumer 根据 active protocol mask 和方向预算运行静态 detector，使用 `natflow_t` 的 8 字节瞬态 context。DNS reply 需要 response header/question 结构证据。 |
...
- 2026-07-26 queue stream 入口：同一用户态工具的 `-w` 模式在 injector 运行期间持续 poll/read，按测试目的端口 bitmap 拒绝重复、遗漏和环境事件；`tests/dpi/run-corpus.sh --queue-stream [cache [generated [parallel]]]` 默认 cache=64/generated=128/parallel=16，producer 分批并发，要求每条事件恰好读一次、结束后队列为空、`matches=events=generated` 且 lost/suppressed 为 0。自动化入口已建立，真机结果仍待执行。
- 2026-07-19 detector corpus 框架：`tests/dpi/run-corpus.sh` 建立两个 IPv4 network namespace，经 root namespace 的真实 FORWARD hook 注入每例独立的 TCP/UDP 连接；`tools/natflow-dpi-traffic.c` 生成 original/reply payload，`tools/natflow-dpi-corpus.c` 先打开 queue，再按 original tuple 过滤并断言 v3 source/app/rule/evidence。runner 拒绝非空 ruleset，退出时恢复 enable/IPv4 forwarding，清理临时 ruleset、iptables 和 namespace；最终 PASS 前会核验 DPI 状态、FORWARD 规则、namespace/veth、forwarding 和临时目录均已恢复。它会改变 generation 和事件统计，只用于隔离测试环境。
