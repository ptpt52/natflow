...
- `DPI_DESIGN.md` Draft v3 把 P2 DPI 定位为有界 L7 元数据分类器：MVP 只做 HTTP Host、TLS/QUIC SNI 到 `app_id` 的映射和被动审计，并从首期包含 `NF_FF_DPI_USE`/`NF_FF_BUSY_USE` gate、统一 parser、终态原因、事务规则、版本化事件和资源硬上限；DNS/JA3/payload/组合规则及 app policy 后置。维护者接受 `nf->status` 非原子 writer 可能导致 owner/QoS/route 位丢失的风险，不设计 path 侧 gate repair；legacy `/proc/sys/urllogger_store/enable` 仍同时控制 URL CSV 和 Host ACL；MVP flow result 只在 `natflow_t` 尾部常驻 `u32 app_id`，其他审计字段只进 terminal event；编码前仍必须确认 bit 21、NATCAP 扩展尾增、conntrack ext probe 和厂商 offload/new-flow 回慢路径契约。
...
- 策略模块在等待认证、URL/SNI/QUIC 解析或 Host ACL 决策时必须设置对应 busy bit，完成后再清除，避免 fast path 提前接管。
