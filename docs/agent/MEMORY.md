...
- fakeuser 的 `ifname` 只允许在 `pf == NFPROTO_NETDEV` 的 path ingress 中按实际 `skb->dev` 更新；普通连接把 hook 的 `pf` 直接传给 `natflow_session_learn()`，函数内部直接检查 `pf == NFPROTO_NETDEV`，再通过 fakeuser 地址匹配当前 conntrack 方向源地址，方向可能是 original 或 reply。无 conntrack 的 IPv4/IPv6 广播或组播也只在 NETDEV ingress 分支早退前通过源 IP 查找已有 fakeuser。PF_INET/PF_INET6、公网侧 ingress、user pre hook 和 ARP/ND MAC 学习不得覆盖。接口变化会追加 userinfo 事件，未启用 NETDEV ingress、path 关闭或尚未创建 fakeuser 时字段允许为空。
