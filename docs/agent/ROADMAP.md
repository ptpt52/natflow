...
实现进度：源码已完成 M0b 的 DPI busy bit、`app_id` 尾增和 layout guard，完成 M0c 的 `natflow_l7` hook lifecycle 骨架、共享 feature/normalize 基础结构、URL hook ops/签名兼容包装迁移、PPPoE normalize/restore 和 packet view 传递、L7 consumer mask/API 与 URL dispatcher、HTTP Host parser、TLS ClientHello/SNI 搜索、TCP TLS SNI cache 生命周期迁移、QUIC Initial header/CRYPTO frame/SNI 搜索迁移、DNS QNAME parser 和 `natflow_l7_host_view` consumer 输入 contract，完成 M0d 的 Host ACL 与 URL record 分配解耦，完成 M1a 的 DPI ctl/queue 设备骨架，完成 M1b 的 domain exact/suffix ruleset、match event producer、DPI host consumer 独立激活和复用 urllogger host 的 `app_id` 写入，完成 M1c 的 DNS QNAME domain 分类和 DNS/SSH/WireGuard protocol-only detector，其中 SSH 支持 TCP 22 和 TCP original-direction `SSH-<version>-` banner，并完成 M1d 的 STUN/TURN、BitTorrent TCP handshake、UDP uTP/DHT 子集、source/reason counters 和 `events_clear` 测试辅助命令；DPI protocol-only hook 当前保持在 `natflow_dpi.c` 独立注册，不合并进 L7 URL common path；QUIC crypto/cache 生命周期迁移、误判 corpus 和生产 shadow 数据尚未实现。
