...
当前进度：`tools/build-matrix.sh` 已固化七组合 clean build；`tools/natflow-dpi-reader.c` 和 `tools/natflow-dpi-ctl-smoke.sh` 已提供 DPI v3 queue 与空 ruleset 控制事务入口。URL parser corpus、QoS、认证状态机和 vline 自动回归仍未建立。
