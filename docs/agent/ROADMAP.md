...
- `/dev/natflow_userinfo_event_queue` 写接口当前返回 `-ENOSYS`。
- `userinfo_ctl`、`natflow_userinfo_event_queue`、`natflow_urllogger_queue` 小 buffer read 当前返回 `-EINVAL`，不是 partial read。
...
目标：把 `userinfo_ctl`、`natflow_userinfo_event_queue`、`natflow_urllogger_queue` 的小 buffer 读取从直接 `-EINVAL` 改为更兼容的 partial read 或 per-open buffer 行为。
