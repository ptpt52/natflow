...
实现进度：源码已完成 M0b 的 DPI busy bit、`app_id` 尾增和 layout guard，完成 M0c 的 `natflow_l7` hook lifecycle 骨架，完成 M0d 的 Host ACL 与 URL record 分配解耦，完成 M1a 的 DPI ctl/queue 设备骨架，完成 M1b 的 domain exact/suffix ruleset、match event producer 和复用 urllogger host 的 `app_id` 写入，并完成 M1c 的 DNS/SSH/WireGuard 端口型 protocol-only detector；legacy URL parser 尚未迁移到共享 feature core，DNS QNAME feature 尚未实现。
