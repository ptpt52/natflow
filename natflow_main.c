...
static void *natflow_start(struct seq_file *m, loff_t *pos)
{
	...
	if ((*pos) == 0) {
		n = snprintf(natflow_ctl_buffer,
		             ...
		             "#    relay_add=<ifname>,<ifname>,<family> -- add relay <family>=ipv4/ipv6/all\n"
			...
			*lpos = (*pos);
			...
			if (ptr) {
				n = snprintf(natflow_ctl_buffer,
				             PAGE_SIZE - 1 - sizeof(unsigned long),
				             "%s_add=%s,%s,%s\n",
				             (family & VLINE_RELAY_MASK) ? "relay" : "vline",
				             (*ptr)[0], (*ptr)[1],
				             vline_fwd_family_name(family & VLINE_FAMILY_MASK));
				natflow_ctl_buffer[n] = 0;
				return natflow_ctl_buffer;
			}
		}
...
static ssize_t natflow_write(struct file *file, const char __user *buf, size_t buf_len, loff_t *offset)
{
	...
	if (strncmp(data, "debug=", 6) == 0) {
		...
		if (n == 3) {
			unsigned char family = vline_fwd_family_parse(family_str);
		...
		if (n == 3) {
			unsigned char family = vline_fwd_family_parse(family_str);
			if (family != VLINE_FAMILY_MASK) {
				err = vline_fwd_map_config_add(str[1], str[0], family | VLINE_RELAY_MASK);
				if (err == 0) {
					goto done;
				}
			}
