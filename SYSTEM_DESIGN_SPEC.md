...
- `app_id` 继续是唯一分类结果；packet/byte counter 和 16 位 discovery/claimed automaton word 是 `natflow_t` 内瞬态 context。所需方向始终没有 payload时，context 可以保留到 conntrack 生命周期结束。
