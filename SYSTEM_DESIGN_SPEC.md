...
- 字符设备：`/dev/natflow_ctl`、`/dev/natflow_zone_ctl`、`/dev/natflow_user_ctl`、`/dev/qos_ctl`、`/dev/userinfo_ctl`、`/dev/natflow_userinfo_event_queue`、`/dev/conntrackinfo_ctl`、`/dev/hostacl_ctl`、`/dev/natflow_urllogger_queue`、`/dev/natflow_dpi_ctl`、`/dev/natflow_dpi_queue`。
...
- `natflow_userinfo_event_queue` 阻塞输出认证二进制事件。
- `natflow_urllogger_queue` 输出 URL/SNI 二进制事件。
- 各控制设备读接口可输出可重放配置。
...
- user 子系统初始化时，`userinfo_event_store_init()` 在 `/dev/natflow_userinfo_event_queue` 的 `cdev_add()` 之前执行；设备节点一旦可打开，事件队列的 spinlock、list 和 waitqueue 已经有效。
...
除 `conntrackinfo_ctl` 的长读逻辑和 `natflow_userinfo_event_queue` 的阻塞读外，大多数字符设备写入均采用同一协议：
...
| `-EBUSY` | `/dev/natflow_userinfo_event_queue`、`/dev/natflow_urllogger_queue` 或 `/dev/natflow_dpi_queue` 已有 reader。 |
| `-ENOSYS` | `/dev/natflow_userinfo_event_queue` 写接口未实现。 |
...
- 不支持 partial read 的队列接口：`userinfo_ctl`、`natflow_userinfo_event_queue`、`natflow_urllogger_queue`；用户 buffer 小于单条记录时返回 `-EINVAL`。
...
### 7.6 `/dev/natflow_userinfo_event_queue`
...
- `read` 阻塞等待用户事件，输出版本化二进制固定头事件；`read` 不再负责启动或停止全局事件流。
...
- 没有 reader 时 URL/SNI record 直接丢弃，不缓存、不增加 `memsize/count`；reader 打开时清空残留记录，关闭时清空未读记录。
...
| `memsize` | 0 | 当前已缓存待读 URL 记录的内存使用，只读语义。 |
...
- `userinfo_ctl`、`natflow_userinfo_event_queue`、`natflow_urllogger_queue` 对小 buffer 不支持 partial read。
