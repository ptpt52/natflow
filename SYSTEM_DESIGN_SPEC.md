...
- DPI 默认 `disabled`。`enable=1` 后，domain 规则激活 `DPI_DOMAIN` consumer，proto 规则激活 `DPI_PACKET` consumer。DNS 和 payload detector 使用静态 metadata 声明 L4、方向模式及双向 packet/byte budget；未命中时，packet consumer 在 `natflow_t` 尾部记录双向计数和 detector mask，保持 `NF_FF_L7_USE | NF_FF_DPI_USE`，而不是立即写 packet done。命中、已有 `app_id`、FIN/RST 或所有 detector 双向预算耗尽时清 context 并终态。当前公共入口仍跳过 reply，因此现阶段只会累加 original 预算。
...
以下是 Draft v7 后续实现合同。packet direction、client/server port、静态 detector metadata dispatcher 和 `natflow_t` 内 bounded direction context 已完成，当前源码尚未放开 conntrack reply direction：
