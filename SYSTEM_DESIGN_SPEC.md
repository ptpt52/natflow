...
| `natflow_dpi.c/.h` | 编译源码/头 | DPI 控制/事件接口，提供默认关闭的 `/dev/natflow_dpi_ctl`、19 个固定应用、DNS QNAME 查询意图、26 个固定原生协议状态机、固定 app/category catalog、L7 packet-view consumer、`app_id` 写入、source counters 和 `/dev/natflow_dpi_queue` match 事件。 |
...
| `DPI_DESIGN.md` | 文档 | 统一 L7 core 的历史基线；分类器目标已由 `DPI_HARDCODED_STATE_MACHINE_DESIGN.md` 和 ADR-0009 修订。当前源码实现 L7 feature core、19 个固定应用、26 个固定原生协议状态机、match event producer、source counters 和固定 `app_id` 写入。 |
...
- `ifname` 的所有权在 user 模块，功能规格依赖 path。普通 TCP/UDP 单播由 user pre hook 通过 `natflow_session_ingress_dev(ct, dir)` 读取当前 generation 下 `rroute[!dir].outdev`：original 方向覆盖用户主动连接，已由 post hook 关联 fakeuser 的外网主动连接在 LAN reply 方向仅当 fakeuser 地址与 reply 源地址一致时刷新已有用户来源并立即退出，避免 LAN-to-LAN/hairpin 应答污染原用户，且不进入认证或重定向流程。字段为空时先于活动节流尝试 path accessor，成功后立即发布事件；普通 MAC/ifname/timeout 刷新仍沿用 32 秒和新连接 2 秒节流。`natflow_get_mac_or_fake()` 在 Ethernet 设备且 skb 已设置 MAC header 时返回真实源 MAC，否则生成确定性的本地管理单播伪 MAC；IPv4 格式为 `02:00:<IPv4 四字节>`，IPv6 的前两字节来自地址四个 32 位分量的 xor，后四字节来自固定 seed `0x12345678` 的 jhash。普通来源刷新把该真实或伪 MAC 写入 `fud->macaddr`，因此 `tun`、`ppp` 等 L3-only 接口也能保存稳定用户身份。对于不建立 route 的广播、组播和 ICMP/ICMPv6，NETDEV ingress 必须保持 path 原有校验及早退顺序；局部 IPv4/IPv6 包装器只在对应早退分支内独立验证 IPv4 total length、header checksum 或 IPv6 payload length，验证通过后才调用 `natflow_user_ingress_ifname_learn(skb, saddr, family)`。该入口仅接受 LAN zone 设备或其 bridge slave，使用同一真实/伪 MAC 规则按源地址校验已有 fakeuser；用户不存在时直接返回，不创建 fakeuser，已有用户 MAC 非零且不匹配时不更新 ifname，验证后记录原始 `skb->dev` 并在 ifname 变化时发布 userinfo 事件。ARP 不经过该 IP 入口；path 未启用、未启 NETDEV ingress 或没有可用入口信息时字段可以为空。
...
- TCP packet-only DPI 路径只用 `pskb_may_pull()` 准备有界前缀：App HTTP 输入最多 512 字节，通用原生协议 parser 只检查前 96 字节。UDP 通常最多 96 字节，121..299 字节的爱奇艺候选拉取完整 datagram。只有 URL 或 DPI domain host consumer 激活时才为既有 HTTP/TLS producer 请求完整 TCP payload 可写。
...
`CONFIG_NATFLOW_DPI` 当前启用默认关闭的控制面、19 个固定应用、DNS QNAME 查询意图分类、A/B 级固定原生协议状态机、固定 app/category catalog、source counters 和 match event 队列：
...
- 固定 protocol app ID 在原有 1..18 后追加 NTP=19、SNMP=20、RADIUS=21、TFTP=22、LDAP=23、NFS=24、SOCKS=25、CoAP=26。固定应用 ID 在原有 9 项后追加腾讯视频=`0x1004`、Spotify=`0x1005`、WhatsApp=`0x2005`、Messenger=`0x2006`、Discord=`0x2007`、Zoom=`0x2008`、Facebook=`0x5002`、Instagram=`0x5003`、X/Twitter=`0x5004`、微博=`0x5005`；catalog revision=3，共 45 项；发布后不改号或复用。
...
- `NF_CONNTRACK` 无条件包含 conntrack accounting 实现和 `nf_conn_acct_find()`，没有独立的 `CONFIG_NF_CONNTRACK_ACCT` 编译符号；运行时 `net.netfilter.nf_conntrack_acct` 决定新连接是否挂 acct 扩展，OpenWrt 默认启用。acct 存在时，DPI packet consumer 在 `ct->lock` 下读取双向原子包计数：总数不超过 256 继续原状态机，第 257 包清除活跃 context、写 packet done，并增加 `context_cleared_acct_limit`。acct 不存在时不执行该兜底。TCP 零负载包也进入此生命周期检查，但不运行 payload parser、不消耗 payload budget。
...
- `/dev/natflow_dpi_ctl` status 中，`matches`/`matches_*` 统计全部 static-domain/fixed-protocol 终态，`events`/`events_*` 只统计成功入队，`events_suppressed` 统计 reader/cache 未启用导致的主动抑制，`events_lost` 统计分配失败或队列满。稳定采样时满足 `matches = events + events_suppressed + events_lost`；新 producer 在 match 与最终 queue outcome 之间可造成短暂不一致，但 `events_clear` 的 quiesce 边界保证复位前 producer 不会在命令返回后写入复位后的区间。`domain_lookups`/`domain_matches` 统计 hostname lookup 和静态表命中，`dns_app_intents` 统计 QNAME 静态应用意图；`packet_inspect_original/reply` 每个实际进入有界 App/协议 parser 的 packet 最多增加一次，`packet_match_original/reply` 记录直接 App/协议证据方向；`context_armed`、各 `context_cleared_*` 和 `context_aborted` 记录 bounded context 累计转换，其中 `context_cleared_acct_limit` 专门记录上述包数兜底。`events_clear` 复位该字段。conntrack 自然销毁不回调 DPI，不能用累计 arm/clear 差值推导当前 active context。`proto_no_session` 和 `proto_app_exists` 解释固定 terminal commit 未产生新结果的原因；不存在 `proto_no_rule`。
- 固定事件头为 packed `struct natflow_dpi_event_hdr`，`version=3`，固定 78 字节，并由编译期约束防止结构长度漂移；包含 `header_len`、`record_len`、`reason`、`generation`、`app_id`、`category_id`、`rule_id`、`flags`、`timestamp`、original tuple 和 `evidence_dir`。所有 event 使用 `generation=NATFLOW_DPI_CATALOG_REVISION=3`、静态 app/category ID 和 `rule_id=0`。`flags=1..36` 表示具体 feature/protocol/App payload source；25..36 依次为 NTP、SNMP、RADIUS、TFTP、LDAP、NFS、SOCKS、CoAP、WhatsApp、Discord、Spotify、Zoom；`evidence_dir=0/1` 分别表示命中证据来自 original/reply。
- `tools/natflow-dpi-event.h` 固化用户态工具共享的 v3/78 字节 event ABI；reader、queue smoke/pressure 和 corpus 工具验证事件与压力路径。traffic/corpus 工具的 TCP sequence 模式可构造双向顺序、反序、同时发送、重复证据、预算耗尽和 transport end；TCP ACK 模式用 repair queue sequence 构造合法的无 payload ACK。`evidence_dir=either` 只用于并发测试断言实际终态方向仍为 original/reply。`tools/natflow-dpi-ctl-smoke.sh` 验证静态控制命令、已删除规则命令、`context_cleared_acct_limit` 复位和 enable 恢复；`tools/build-matrix.sh` 对七种基础/URL/DPI/PATH/NO_DEBUG 组合逐项 clean build。这些工具是回归入口，不是常驻用户态控制面。
- `tests/dpi/run-corpus.sh` 是原生协议机器黑盒 corpus 入口：默认使用两个 IPv4 network namespace，`--ipv6` 使用两个 IPv6 `/64`，让 fixture 经过 root namespace 的 FORWARD hook并断言固定 catalog revision、app/category、`rule_id=0` 和 evidence direction。`--packet-limit` 临时启用并恢复 conntrack accounting，断言 UDP 同 flow 第 256/257 包边界、counter reset 和纯 TCP ACK 生命周期路径。测试修改 forwarding、iptables/ip6tables、DPI enable 和事件统计，只适用于隔离测试环境；不再有空 ruleset 前置条件。
...
- 单包原生机器直接从 parser 结果返回 `PENDING` 或 `TERMINAL`；等待 reply、任一方向后续证据或跨包事实时使用 `natflow_t` 尾部 8 字节 context，并设置 `NF_FF_DPI_USE`。当前 RDP、SOCKS 和 WhatsApp 持久认领 compact automaton machine/state。
