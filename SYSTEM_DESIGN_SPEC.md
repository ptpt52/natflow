...
- `natflow_userinfo_queue` 输出认证二进制事件，空队列 `read()` 返回 0，并支持 `poll()`。
...
| `-ENOSYS` | `/dev/natflow_userinfo_queue` 写接口未实现。 |
...
- `release` 会把 `stage` 置回 STOPPED，唤醒等待中的 poller，并清空全局待消费事件队列。
- `write` 返回 `-ENOSYS`。
...
- `userinfo_ctl`、`natflow_userinfo_queue`、`natflow_urllogger_queue` 对小 buffer 不支持 partial read。
