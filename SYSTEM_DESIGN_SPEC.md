...
| `natflow_dpi.c/.h` | 编译源码/头 | DPI 控制/事件接口，提供默认关闭的 `/dev/natflow_dpi_ctl`、YouTube/Netflix/Telegram 静态域名应用状态机、迁移期 domain ruleset、DNS QNAME 查询意图、18 个固定 protocol detector、固定 app/category catalog、L7 packet-view consumer、`app_id` 写入、source counters 和 `/dev/natflow_dpi_queue` match 事件。 |
...
| `DPI_DESIGN.md` | 文档 | 统一 L7 core 的历史基线；分类器目标已由 `DPI_HARDCODED_STATE_MACHINE_DESIGN.md` 和 ADR-0009 修订。当前源码实现 L7 feature core、3 个静态域名应用、迁移期 domain ruleset、18 个固定 protocol detector、match event producer、source counters 和固定 `app_id` 写入。 |
...
| `CONFIG_NATFLOW_DPI` | 编译并初始化 DPI 控制/事件接口，提供 `/dev/natflow_dpi_ctl`、3 个静态域名应用、迁移期 domain ruleset、DNS QNAME 查询意图、18 个固定 protocol detector 和 `/dev/natflow_dpi_queue`；默认关闭。DPI enabled 即激活 L7 DPI host/packet consumer，不要求规则或 `/proc/sys/urllogger_store/enable`。 |
...
`CONFIG_NATFLOW_DPI` 当前启用默认关闭的控制面、YouTube/Netflix/Telegram 静态域名应用状态机、迁移期 domain exact/suffix ruleset、DNS QNAME 查询意图分类、A/B 级固定 protocol detector、固定 app/category catalog、source counters 和 match event 队列：
...
- 静态域名状态机由 HTTP Host、TLS SNI 和 QUIC v1 Initial SNI 驱动，先 exact，再按长度从长到短做 label-boundary suffix 匹配。表项和固定 app metadata 在模块初始化时校验顺序、重复项和引用关系；热路径最多遍历当前 14 个域名项，命中后 O(1) 取得固定 app metadata。静态结果优先于迁移期用户 domain rule。
- DNS QNAME detector 在 original direction 的 TCP/UDP 53 标准 query 中解析第一问 QNAME，支持有界 compression pointer 展开；最多跳转 16 次并记录已访问 offset，拒绝环、越界、非法 label 和展开后超长名称。QNAME 经过同一静态 matcher，但只记录 `dns_app_intents` 查询意图，不把目标应用写入 DNS flow；该 flow 仍终态为 DNS。DNS QNAME 不再进入迁移期用户 domain rule。
...
- 静态 domain app 或固定 protocol 命中时通过统一 commit helper 对当前连接的 `natflow_t.app_id` 执行 `cmpxchg()`，仅第一个从 0 变为固定 app ID 的结果产生事件；L7 入口已经在解析前统一确保 natflow session。迁移期 domain rule 同样只能填充空 app，不覆盖静态结果。
...
- `/dev/natflow_dpi_ctl` status 中，`matches`/`matches_*` 统计全部 static-domain/dynamic-domain/fixed-protocol 终态，`events`/`events_*` 只统计成功入队，`events_suppressed` 统计 reader/cache 未启用导致的主动抑制，`events_lost` 统计分配失败或队列满。没有并发读数或 reset 时满足 `matches = events + events_suppressed + events_lost`。`domain_lookups`/`domain_matches` 统计 hostname lookup 和静态/迁移期表命中，`dns_app_intents` 统计 QNAME 静态应用意图；`packet_inspect_original/reply` 每个实际进入有界 protocol parser 的 packet 最多增加一次，`packet_match_original/reply` 记录直接协议证据方向；`context_armed`、各 `context_cleared_*` 和 `context_aborted` 记录 bounded context 累计转换。conntrack 自然销毁不回调 DPI，不能用累计 arm/clear 差值推导当前 active context。`proto_no_session` 和 `proto_app_exists` 解释固定 terminal commit 未产生新结果的原因；不存在 `proto_no_rule`。
- 固定事件头为 packed `struct natflow_dpi_event_hdr`，`version=3`，固定 78 字节，并由编译期约束防止结构长度漂移；包含 `header_len`、`record_len`、`reason`、`generation`、`app_id`、`category_id`、`rule_id`、`flags`、`timestamp`、original tuple 和 `evidence_dir`。静态域名应用与固定 protocol event 使用 `generation=NATFLOW_DPI_CATALOG_REVISION=1`、静态 app/category ID 和 `rule_id=0`；迁移期 domain event 继续使用 ruleset generation、用户 app/rule ID 和 `category_id=0`。`flags=1..9` 依次表示 HTTP、TLS、QUIC、DNS、SSH、WireGuard、STUN、TURN、BitTorrent，`flags=10..21` 依次表示 FTP、SMTP、POP3、IMAP、SIP、RTSP、MQTT、RESP、MySQL、PostgreSQL、RDP、SMB。`timestamp` 为 uptime 秒数，与 URL logger 事件语义一致。tuple 字段固定取 `IP_CT_DIR_ORIGINAL`，`family` 为 `AF_INET` 或 `AF_INET6`，`l4proto` 为 L4 protocol，`sport`/`dport` 为主机字节序端口，`sip`/`dip` 为地址字节数组；IPv4 使用前 4 字节，IPv6 使用完整 16 字节。`evidence_dir=0/1` 分别表示命中证据来自 original/reply；domain host event 固定为 original，固定 protocol event 记录实际命中 packet 方向。
