...
- 字符设备：`/dev/natflow_ctl`、`/dev/natflow_zone_ctl`、`/dev/natflow_user_ctl`、`/dev/qos_ctl`、`/dev/userinfo_ctl`、`/dev/userinfo_event_ctl`、`/dev/conntrackinfo_ctl`、`/dev/hostacl_ctl`、`/dev/natflow_urllogger_queue`、`/dev/natflow_dpi_ctl`、`/dev/natflow_dpi_queue`。
...
| `-EBUSY` | `/dev/userinfo_event_ctl`、`/dev/natflow_urllogger_queue` 或 `/dev/natflow_dpi_queue` 已有 reader。 |
...
- 不支持 partial read 的队列接口：`userinfo_ctl`、`userinfo_event_ctl`、`natflow_urllogger_queue`；用户 buffer 小于单条记录时返回 `-EINVAL`。
...
- `userinfo_ctl`、`userinfo_event_ctl`、`natflow_urllogger_queue` 对小 buffer 不支持 partial read。
