...
| `natflow_dpi.c/.h` | 编译源码/头 | DPI 控制/事件接口，提供默认关闭的 `/dev/natflow_dpi_ctl`、domain exact/suffix ruleset、DNS/SSH/WireGuard protocol-only ruleset、`app_id` 写入和 `/dev/natflow_dpi_queue` match 事件。 |
...
| `DPI_DESIGN.md` | 文档 | 统一 L7 core 与 DPI 目标设计，覆盖 legacy URL/HostACL consumer、DPI classifier consumer、HTTP/TLS/QUIC 共享解析、`app_id` flow result、独立 DPI ABI、分级 detector 和 M0-M4 实施路径；当前源码已预留 DPI busy bit、`app_id`、layout guard、L7 hook lifecycle 骨架，并实现 DPI domain exact/suffix ruleset、DNS/SSH/WireGuard protocol-only ruleset、match event producer 和 `app_id` 写入。 |
...
| `CONFIG_NATFLOW_DPI` | 编译并初始化 DPI 控制/事件接口，提供 `/dev/natflow_dpi_ctl`、domain exact/suffix ruleset、DNS/SSH/WireGuard protocol-only ruleset 和 `/dev/natflow_dpi_queue`；默认关闭。当前 HTTP/TLS/QUIC host 分类复用 `CONFIG_NATFLOW_URLLOGGER` parser。 |
...
- `app_id`：DPI 的常驻应用分类结果；`0` 表示 unknown、未分类或没有结果。当前 domain exact/suffix matcher 和 DNS/SSH/WireGuard protocol-only matcher 在规则命中且已有 natflow session 时写入非 0 `app_id`，其他 DPI 细节只进入事件。
...
`CONFIG_NATFLOW_DPI` 当前启用默认关闭的控制面、domain exact/suffix ruleset、DNS/SSH/WireGuard protocol-only ruleset 和 match event 队列：

- `/dev/natflow_dpi_ctl` 使用 seq_file 输出状态，支持 `enable=0|1`、`enable`、`disable`、`rules_begin`、`domain id=<rule_id> app=<app_id> kind=exact|suffix host=<host>`、`proto id=<rule_id> app=<app_id> proto=dns|ssh|wireguard`、`rules_commit`、`rules_abort` 和 `rules_clear`。
...
- `proto` 当前支持 `dns`、`ssh`、`wireguard`/`wg`。protocol-only detector 当前按 original direction 的目标端口识别：DNS TCP/UDP 53，SSH TCP 22，WireGuard UDP 51820。
- DPI 默认 `disabled`。`enable=1` 后，HTTP/TLS/QUIC host 分类仍来自 legacy URL logger parser，因此需要同时编译并启用 `CONFIG_NATFLOW_URLLOGGER` 和 `/proc/sys/urllogger_store/enable=1`；DNS/SSH/WireGuard 由 `natflow_dpi.c` 自己的 IPv4、IPv6、bridge FORWARD hook 处理，优先级 `NF_IP_PRI_FILTER + 6`。
...
- 固定事件头为 packed `struct natflow_dpi_event_hdr`，`version=1`，包含 `header_len`、`record_len`、`reason`、`generation`、`app_id`、`category_id`、`rule_id`、`flags` 和 `timestamp`。当前 match event 使用 `reason=6`，`category_id=0`，`flags` 表示来源：1=HTTP、2=TLS、3=QUIC、4=DNS、5=SSH、6=WireGuard，`timestamp` 为 `ktime_get_ns()`。
