...
- `tools/natflow-dpi-event.h` 固化用户态工具共享的 v3/78 字节 event ABI；`tools/natflow-dpi-reader.c` 是可编译参考消费者；`tools/natflow-dpi-queue-smoke.c` 验证单 reader、不可 seek、小 buffer、空队列、cache/close 语义，并可等待真实事件验证固定头；`tools/natflow-dpi-queue-pressure.c` 在单 reader 暂停读取期间执行外部并发 injector，排空后验证 cache 上限内的事件 ABI 和测试 tuple；`tests/dpi/run-corpus.sh --queue-pressure` 负责建立隔离拓扑并核验 matches/events/suppressed/lost 及 source 分项；`tools/natflow-dpi-ctl-smoke.sh` 在空 ruleset 测试环境验证控制事务并恢复 enable；`tools/build-matrix.sh` 对七种基础/URL/DPI/PATH/NO_DEBUG 组合逐项 clean build。这些工具是回归入口，不是常驻用户态控制面。
