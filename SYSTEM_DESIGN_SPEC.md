...
- `tests/dpi/run-corpus.sh` 是 detector 黑盒 corpus 入口：默认使用两个 IPv4 network namespace，`--ipv6` 则使用两个 IPv6 `/64`，让 fixture 经过 root namespace 的 FORWARD hook，临时安装固定测试规则，并通过支持双栈地址的 `tools/natflow-dpi-corpus.c` 和 `tools/natflow-dpi-traffic.c` 注入双向 TCP/UDP payload、过滤 4/16 字节 original tuple、断言分类结果。它要求空 ruleset 并修改对应 family 的 forwarding、iptables/ip6tables、DPI generation/enable 和事件统计，只适用于隔离测试环境；最终 PASS 前会核验 DPI 状态、FORWARD 规则、namespace/veth、forwarding 和临时目录均已恢复。IPv6 模式当前不构造 extension header。
