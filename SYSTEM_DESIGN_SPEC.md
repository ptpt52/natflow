...
- 字符设备：`/dev/natflow_ctl`、`/dev/natflow_zone_ctl`、`/dev/natflow_user_ctl`、`/dev/qos_ctl`、`/dev/userinfo_ctl`、`/dev/natflow_userinfo_queue`、`/dev/conntrackinfo_ctl`、`/dev/hostacl_ctl`、`/dev/natflow_urllogger_queue`、`/dev/natflow_dpi_ctl`、`/dev/natflow_dpi_queue`。
...
除 `conntrackinfo_ctl` 的长读逻辑外，大多数字符设备写入均采用同一协议：
...
- 不支持 partial read 的队列接口：`userinfo_ctl`、`natflow_userinfo_queue`、`natflow_urllogger_queue`、`natflow_dpi_queue`；用户 buffer 小于单条记录时返回 `-EINVAL`。
...
- `read` 每次从全局队列取出一条事件并输出版本化二进制固定头事件；队列为空时返回 0，不挂起。
...
- 队列为空或首条记录未超过 `timestamp_freq` 老化阈值时，`read()` 返回 0；`poll()` 只在首条记录已达到读出条件时返回 readable。由于没有为老化到期单独挂 timer，长期 reader 应保持 fd 打开并使用带超时的 `poll()`/`select()`/`epoll` 循环。
...
- URL logger 输出版本化二进制事件，payload 是未 NUL 结尾的 `host + uri`；用户态必须按 `record_len/header_len/host_len` 解析，并处理小 buffer 返回 `-EINVAL` 等限制。
...
- `userinfo_ctl`、`natflow_userinfo_queue`、`natflow_urllogger_queue`、`natflow_dpi_queue` 对小 buffer 不支持 partial read。
