...
- `tools/natflow-dpi-event.h` 固化用户态工具共享的 v3/78 字节 event ABI；`tools/natflow-dpi-reader.c` 是可编译参考消费者；`tools/natflow-dpi-queue-smoke.c` 验证单 reader、不可 seek、小 buffer、空队列、cache/close 语义，并可等待真实事件验证固定头；`tools/natflow-dpi-ctl-smoke.sh` 在空 ruleset 测试环境验证控制事务并恢复 enable；`tools/build-matrix.sh` 对七种基础/URL/DPI/PATH/NO_DEBUG 组合逐项 clean build。这些工具是回归入口，不是常驻用户态控制面。
