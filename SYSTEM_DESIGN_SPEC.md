...
- DPI 默认 `disabled`。reply 包进入公共入口后，consumer mask 强制收窄为 `DPI_PACKET`；URL、Host ACL、HTTP/TLS/QUIC host 和 DNS QNAME domain 不消费 reply。DNS protocol reply 需要通过 response header、标准 opcode、question count 和未压缩第一问结构校验；端口本身不分类。未命中时 packet consumer在 `natflow_t` 尾部记录双向计数和 detector mask。
