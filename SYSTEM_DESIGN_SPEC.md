...
- `natflow_ct_ext_layout_validate()` 会验证 `NF_FF_L7_USE_BIT=19`、`NF_FF_DPI_USE_BIT=21`、`NF_FF_L7_URL_DONE_BIT=22`、`NF_FF_L7_DPI_DOMAIN_DONE_BIT=23`、`NF_FF_L7_DPI_PACKET_DONE_BIT=24`、`NF_FF_BUSY_USE` 包含 L7/DPI busy bit 且不包含 done bit、`nat_key_t` 偏移不超过 `NATCAP_MAX_OFF`、`natflow_off` 可放入 `nat_key_t` 的 16 位字段，并确认 `natflow_t` 的对齐后长度覆盖新增 `app_id`。
...
- DNS QNAME detector 在 original direction 的 TCP/UDP 53 标准 query 中解析第一问 QNAME，忽略 response、非 query opcode、压缩 QNAME、畸形或前缀不足的报文；QNAME 经过同一 hostname normalize 后进入 domain exact/suffix ruleset，有效标准 query 也可命中 DNS protocol-only 规则，命中事件 source 为 DNS。
- 端口只选择有界解析候选和 payload pull budget，不直接写入 `app_id`；当前 TCP/UDP 53 用作 DNS 候选，TCP 22 和 UDP 51820 不再单独构成 SSH/WireGuard 分类证据。
- 有界 payload detector 当前覆盖 TCP original direction 的 SSH banner `SSH-<version>-` identification string、WireGuard UDP message type/reserved bytes/长度校验、STUN/TURN header、length 和 magic cookie，按 TURN 方法区分 TURN；BitTorrent 的 TCP 分支覆盖标准 handshake，UDP 分支覆盖 uTP v1 header 和 DHT bencode token 前缀窗口，其中 uTP 会校验版本、类型和扩展号。数据面根据 active ruleset 的 protocol mask 只运行已配置 detector。IPv6 detector 当前只处理无 extension header 的 TCP/UDP。
- DPI 默认 `disabled`。`enable=1` 后，domain 规则激活 `NATFLOW_L7_CONSUMER_DPI_DOMAIN`，proto 规则激活 `NATFLOW_L7_CONSUMER_DPI_PACKET`，二者组合为 `NATFLOW_L7_CONSUMER_DPI`。L7 shared hook 构造 packet view 并调度 `natflow_dpi_consume_packet_view()`，该函数返回本次可终态的 DPI domain/packet mask。L7 producer 在 TCP/UDP 分发点统一解析 L3/L4 header，并按 DPI 需要拉取 DNS 或 payload detector 的有界前缀，把 conntrack direction、当前 packet `sport/dport`、`payload_len` 和 `payload_linear_len` 传给 DPI；packet view 同时提供方向感知的 client/server port helper。DPI consumer 不再自行按 IPv4/IPv6 重解析 skb。DNS 和 payload detector 已使用编译期静态 metadata 声明 L4、方向模式及 original/reply packet/byte budget；payload dispatcher 按固定优先级最多遍历 4 个 detector family，pull API 根据 active protocol mask、consumer mask、direction 和 server port 决定窗口。当前公共入口填充 direction 后仍跳过 reply，HTTP/TLS/QUIC host 分类、DNS QNAME domain 分类和 protocol-only detector 都只消费 original packet，且不依赖 `/proc/sys/urllogger_store/enable`；非 DNS payload detector 只在存在 proto 规则时执行。
...
以下是 Draft v7 后续实现合同。packet direction、client/server port 和静态 detector metadata dispatcher 基础设施已经完成，当前源码尚未实现 bounded direction context 或放开 conntrack reply direction：
