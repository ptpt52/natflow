...
- `natflow_userinfo_queue` 阻塞输出认证二进制事件。
...
除 `conntrackinfo_ctl` 的长读逻辑和 `natflow_userinfo_queue` 的阻塞读外，大多数字符设备写入均采用同一协议：
...
- 不支持 partial read 的队列接口：`userinfo_ctl`、`natflow_userinfo_queue`、`natflow_urllogger_queue`；用户 buffer 小于单条记录时返回 `-EINVAL`。
