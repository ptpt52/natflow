...
- 当前实际目录中没有 `natflow_path.c.orig`、`cscope.files`、`cscope.out` 等历史备份或索引文件；`.gitignore` 仍会忽略 `*.orig` 和 `cscope.*`，因此未来若这些文件重新出现，应先判断是否为生成物或临时备份。
...
- 当前 Git 工作区的源码和文档清单不包含历史 `.orig` 或 cscope 索引文件；后续审阅时若看到这类文件，应按生成物/备份文件处理，不能自动纳入构建规格。
