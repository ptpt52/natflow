...
| `natflow_dpi.c/.h` | 编译源码/头 | DPI 控制/事件接口，提供默认关闭的 `/dev/natflow_dpi_ctl`、domain exact/suffix ruleset、DNS QNAME domain 分类、18 个固定 protocol detector、固定 app/category catalog、L7 packet-view consumer、`app_id` 写入、source counters 和 `/dev/natflow_dpi_queue` match 事件。DPI consumer 只消费 L7 packet view 中的 L4/payload 字段，不再自行重解析 IPv4/IPv6 skb。 |
...
| `natflow_conntrack.c/.h` | 编译源码/头 | `/dev/natflow_conntrackinfo_ctl` conntrack dump。 |
...
| `DPI_DESIGN.md` | 文档 | 统一 L7 core 的历史基线；分类器目标已由 `DPI_HARDCODED_STATE_MACHINE_DESIGN.md` 和 ADR-0009 修订。当前源码实现 L7 feature core、DPI domain exact/suffix ruleset、DNS QNAME domain 分类、18 个固定 protocol detector、L7 shared hook packet-view 调度、match event producer、source counters 和固定 `app_id` 写入。 |
...
| `CONFIG_NATFLOW_DPI` | 编译并初始化 DPI 控制/事件接口，提供 `/dev/natflow_dpi_ctl`、domain exact/suffix ruleset、DNS QNAME domain 分类、18 个固定 protocol detector 和 `/dev/natflow_dpi_queue`；默认关闭。DPI enabled 即激活 L7 DPI packet consumer，domain rule 非空时另激活 domain consumer；HTTP/TLS/QUIC host、DNS QNAME 和 protocol detector 均从 L7 shared hook 的 packet view 入口消费，不依赖 `/proc/sys/urllogger_store/enable`。 |
...
启用 `CONFIG_NATFLOW_URLLOGGER` 或 `CONFIG_NATFLOW_DPI` 时，编译目标必须满足
`THREAD_SIZE >= 8192`；更小的内核线程栈会由 `natflow_l7.c` 在编译期拒绝。
Kbuild 对 `natflow_l7.o`、`natflow_dpi.o` 和 `natflow_urllogger.o` 追加
`-Werror=frame-larger-than=512`，不依赖调用方是否覆盖全局 `EXTRA_CFLAGS`，
单函数栈帧超过 512 字节会直接构建失败。该阈值只约束单帧，累计调用链仍
必须结合目标工具链生成的 `.su` 文件审核。
...
- 字符设备：`/dev/natflow_ctl`、`/dev/natflow_zone_ctl`、`/dev/natflow_user_ctl`、`/dev/natflow_qos_ctl`、`/dev/natflow_userinfo_ctl`、`/dev/natflow_userinfo_queue`、`/dev/natflow_conntrackinfo_ctl`、`/dev/hostacl_ctl`、`/dev/natflow_urllogger_queue`、`/dev/natflow_dpi_ctl`、`/dev/natflow_dpi_queue`。
...
- `natflow_conntrackinfo_ctl` 输出非 fakeuser/non-NATCAP-peer 的 conntrack 文本。
- `natflow_userinfo_ctl` 输出 fakeuser 用户状态和计数。
- `natflow_userinfo_queue` 输出认证二进制事件，默认不缓存，reader 写入正数 `cache=N` 后最多缓存 N 条，空队列 `read()` 返回 0，并支持 `poll()`。
- `natflow_urllogger_queue` 输出 URL/SNI 二进制事件，默认不缓存，reader 写入正数 `cache=N` 后最多缓存 N 条。
- `natflow_dpi_queue` 输出 DPI match 二进制事件，默认不缓存，reader 写入正数 `cache=N` 后最多缓存 N 条。
- 各控制设备读接口可输出可重放配置。
...
| `-EBUSY` | `/dev/natflow_userinfo_queue`、`/dev/natflow_urllogger_queue` 或 `/dev/natflow_dpi_queue` 已有 reader。 |
...
| `https_redirect_port=<u>` | 设置 HTTPS DNAT 端口，合法范围 `0..65535`，内部保存为网络字节序。 |
...
### 7.6 `/dev/natflow_userinfo_queue`
...
- `cache=N`：N 为十进制无符号整数；正数 N 设置最多缓存 N 条新 URL/SNI record。
- `cache=0`：关闭缓存并清空 URL store。

读接口：

- 每次取出并删除一个或多个超过 `timestamp_freq` 老化阈值的 URL 记录。
- 只允许一个 reader，第二个 reader 打开返回 `-EBUSY`。
- 没有 reader 或 reader 未写入正数 `cache=N` 时 URL/SNI record 直接丢弃，不缓存、不增加 `count`；reader 打开时 cache 默认为 0 并清空残留记录，正数 `cache=N` 最多缓存 N 条记录，队列满时丢弃新记录，`cache=0` 或关闭时清空未读记录。
- 队列为空或首条记录未超过 `timestamp_freq` 老化阈值时，`read()` 返回 0；用户 buffer 小于首条 ready 记录时返回 `-EINVAL`，如果已经拷贝了至少一条完整记录且剩余空间放不下下一条 ready 记录，则返回已拷贝字节数并保留下一条。`poll()` 只在首条记录已达到读出条件时返回 readable。由于没有为老化到期单独挂 timer，长期 reader 应保持 fd 打开并使用带超时的 `poll()`/`select()`/`epoll` 循环。
- 输出格式为版本化二进制记录，固定头后直接跟 payload：
...
- `method` 数值：0 NONE，1 GET，2 POST，3 HEAD。
- `source` 数值：1 HTTP，2 TLS/HTTPS SNI，3 QUIC。
- `acl_action` 数值：0 accept/record，1 drop，2 reset，3 redirect。
- payload 长度为 `record_len - header_len`，内容为 `host + uri` 且不带结尾 NUL；`host_len` 标出 host 部分长度，剩余部分为 HTTP URI。TLS/QUIC 记录通常没有 URI。

限制：

- 如果没有满足老化条件的记录，read 返回 0。
- 如果用户 buffer 小于固定头或当前完整记录，返回 `-EINVAL`；记录会保留在队列中，等待更大的 buffer 读取。
- 如果编码后的 `host + uri + NUL` 超过内部 `URLLOGGER_DATALEN`，该记录在创建阶段被拒绝，不会进入队列。
- 内部 `URLLOGGER_DATALEN` 当前为 2048，包含 URL record 内部尾随 NUL；用户态读取 buffer 应能容纳固定头加最多 2047 字节 payload。
...
- `NF_FF_L7_USE`：shared L7 parser 正在等待 HTTP/TLS/QUIC 解析、URL consumer 或 DPI host consumer terminal，fast path 必须暂停。
...
1. 若 URL、DPI domain 和 DPI packet consumer 都未激活，则直接 accept；`/proc/sys/urllogger_store/enable=1` 激活 URL/HostACL consumer，DPI `enable=1` 直接激活固定 protocol packet consumer，存在 domain rule 时另激活 DPI domain consumer。
...
8. L7 dispatcher 在进入 legacy URL consumer 前拦截 UDP/443 并解析 QUIC SNI：
   ...
   - QUIC crypto 上下文按 CPU 分配，并把 key/iv/header-protection key/mask/nonce、HKDF scratch 和 shash desc 缓冲放在 `natflow_l7_quic_crypto_ctx` 中，避免在 `CONFIG_VMAP_STACK` 内核上把栈地址传给 scatterlist/crypto API，同时降低包处理路径栈占用。
   - 不解析 HTTP/3 `:authority` 或 path，不支持 ECH 内层真实 SNI。
   - QUIC crypto 初始化失败时，URL logger 仍可加载，但 QUIC hostname parser 被禁用。
9. 命中 host 后按 active consumer fan-out：URL consumer 执行 URL event 和 Host ACL，正常路径复用 URL record；若 URL record 分配失败，则退到最小 ACL view 尽量执行 ACL，但不会生成对应 `/dev/natflow_urllogger_queue` 记录；若 `/dev/natflow_urllogger_queue` 没有 reader，URL record 在 ACL/DPI 处理后直接丢弃。DPI host consumer 调用 domain classifier，写入 `app_id` 并输出 match event；DPI-only 时不创建 URL record、不执行 Host ACL。当前 HTTP/TLS/QUIC packet producer 已收敛到 `natflow_l7.c`，host fan-out 通过 `natflow_l7_host_view` 统一承载 source、host、URI 和 HTTP method，并交给 legacy URL consumer 的公共 helper 处理 URL record、Host ACL 和 DPI classify。
...
### 15.4 DPI domain rules 与固定 protocol MVP

`CONFIG_NATFLOW_DPI` 当前启用默认关闭的控制面、domain exact/suffix ruleset、DNS QNAME domain 分类、A/B 级固定 protocol detector、固定 app/category catalog、source counters 和 match event 队列：

- `/dev/natflow_dpi_ctl` 使用 seq_file 输出状态，支持 `enable=0|1`、`enable`、`disable`、`rules_begin`、`domain id=<rule_id> app=<app_id> kind=exact|suffix host=<host>`、`rules_commit`、`rules_abort`、`rules_clear` 和 `events_clear`；`proto ...` 已删除并返回 `-EINVAL`。状态额外输出固定 `catalog_revision` 和 `catalog_apps`。
- `rules_begin` 分配 pending domain ruleset，`domain ...` 只能在事务中写入；`rules_commit` 用 RCU 原子发布 domain ruleset 并递增 generation；`rules_abort` 丢弃 pending；`rules_clear` 发布空 domain ruleset。
- `events_clear` 清空 `/dev/natflow_dpi_queue` 中已排队事件，并把全部 match、event、domain、packet、context 和 `proto_*` shadow counters 归零；不改变 enable 状态、ruleset 或 generation。持续流量下可能立刻产生新计数和事件，单项测试前应先暂停流量或临时禁用 DPI。
- 单个 ruleset 当前最多 128 条 domain 规则。`id` 和 `app` 必须非 0，同一事务内 `id` 不能重复；`host` 会转小写、去掉末尾点，并校验 DNS label；`kind=suffix` 匹配完全相同 host 或带点边界的子域名。
- 固定 protocol app ID 为 DNS=1、SSH=2、WireGuard=3、STUN=4、TURN=5、BitTorrent=6、FTP=7、SMTP=8、POP3=9、IMAP=10、SIP=11、RTSP=12、MQTT=13、RESP=14、MySQL=15、PostgreSQL=16、RDP=17、SMB=18；发布后不改号或复用。
- DNS QNAME detector 在 original direction 的 TCP/UDP 53 标准 query 中解析第一问 QNAME，支持有界 compression pointer 展开；最多跳转 16 次并记录已访问 offset，拒绝环、越界、非法 label 和展开后超长名称。QNAME 经过同一 hostname normalize 后进入 domain exact/suffix ruleset。
...
- `natflow_l7` 统一持有 shared hook 入口、`NATFLOW_L7_CONSUMER_URL/DPI_DOMAIN/DPI_PACKET` mask 和 packet dispatcher；active mask 按 `/proc/sys/urllogger_store/enable` 发布 URL consumer，按 DPI enable 发布固定 protocol packet consumer，按 domain rule count 发布 DPI domain consumer。L7 入口先检查 `IPS_NATFLOW_L7_HANDLED` L7_SKIP hint，未命中时再统一调用 `natflow_session_in()`，然后使用 `natflow_t.status` 中的 `NF_FF_L7_URL_DONE`、`NF_FF_L7_DPI_DOMAIN_DONE` 和 `NF_FF_L7_DPI_PACKET_DONE` 独立记录终态；若无法创建 session，则 fail-open 跳过解析，不产生无状态 URL/DPI terminal。`IPS_NATFLOW_L7_HANDLED` 只是从当前 active consumer 全部 done 派生出来的短路缓存，不替代 per-consumer done bit；运行时开启新的 URL/DPI consumer 或提交新 domain 规则不会自动重新武装已设置 L7_SKIP 的旧连接。
- DPI enable、URL enable、domain `rules_commit` 和 `rules_clear` 只改变后续数据包看到的 active consumer/domain ruleset。控制面不枚举 conntrack，不强制完成、不退出、也不清理已经设置 `NF_FF_L7_USE` 或 consumer done bit 的连接；已设置 L7_SKIP 的连接不重新武装，仍在自然 domain 分类路径中的连接若再次分类则读取当时发布的 active domain ruleset。既有连接可以自然终态，也可以保留原 L7 状态直到 conntrack 生命周期结束。
...
- domain 或固定 protocol 命中时写入当前连接的 `natflow_t.app_id` 并输出 match event；L7 入口已经在解析前统一确保 natflow session。若 session 创建失败，则本次 L7 解析整体跳过，因此不会输出无状态 match event。protocol commit 使用 `cmpxchg()`，仅第一个从 0 变为固定 app ID 的结果产生事件。
...
- `/dev/natflow_dpi_ctl` status 中，`matches`/`matches_*` 统计全部 domain/fixed-protocol 命中，`events`/`events_*` 只统计成功入队，`events_suppressed` 统计 reader/cache 未启用导致的主动抑制，`events_lost` 统计分配失败或队列满。没有并发读数或 reset 时满足 `matches = events + events_suppressed + events_lost`。`domain_lookups`/`domain_matches` 统计 hostname rule lookup；`packet_inspect_original/reply` 每个实际进入有界 protocol parser 的 packet 最多增加一次，`packet_match_original/reply` 记录直接协议证据方向；`context_armed`、各 `context_cleared_*` 和 `context_aborted` 记录 bounded context 累计转换。conntrack 自然销毁不回调 DPI，不能用累计 arm/clear 差值推导当前 active context。`proto_no_session` 和 `proto_app_exists` 解释固定 protocol 未产生新结果的原因；不存在 `proto_no_rule`。
- 固定事件头为 packed `struct natflow_dpi_event_hdr`，`version=3`，固定 78 字节，并由编译期约束防止结构长度漂移；包含 `header_len`、`record_len`、`reason`、`generation`、`app_id`、`category_id`、`rule_id`、`flags`、`timestamp`、original tuple 和 `evidence_dir`。domain event 继续使用 ruleset generation、用户 app/rule ID 和 `category_id=0`；固定 protocol event 使用 `generation=NATFLOW_DPI_CATALOG_REVISION=1`、静态 app/category ID 和 `rule_id=0`。`flags=1..9` 依次表示 HTTP、TLS、QUIC、DNS、SSH、WireGuard、STUN、TURN、BitTorrent，`flags=10..21` 依次表示 FTP、SMTP、POP3、IMAP、SIP、RTSP、MQTT、RESP、MySQL、PostgreSQL、RDP、SMB。`timestamp` 为 uptime 秒数，与 URL logger 事件语义一致。tuple 字段固定取 `IP_CT_DIR_ORIGINAL`，`family` 为 `AF_INET` 或 `AF_INET6`，`l4proto` 为 L4 protocol，`sport`/`dport` 为主机字节序端口，`sip`/`dip` 为地址字节数组；IPv4 使用前 4 字节，IPv6 使用完整 16 字节。`evidence_dir=0/1` 分别表示命中证据来自 original/reply；domain host event 固定为 original，固定 protocol event 记录实际命中 packet 方向。
- `tools/natflow-dpi-event.h` 固化用户态工具共享的 v3/78 字节 event ABI；`tools/natflow-dpi-reader.c` 是可编译参考消费者；`tools/natflow-dpi-queue-smoke.c` 验证单 reader、不可 seek、小 buffer、空队列、cache/close 语义，并可等待真实事件验证固定头；`tools/natflow-dpi-queue-pressure.c` 支持两种外部 injector：暂停读取后验证 cache/drop-new，或在 injector 运行期间持续 poll/read 并验证每个测试端口恰好出现一次；`tests/dpi/run-corpus.sh --queue-pressure|--queue-stream` 负责建立隔离拓扑并核验 matches/events/suppressed/lost 及 source 分项；`tools/natflow-dpi-ctl-smoke.sh` 在空 ruleset 测试环境验证控制事务并恢复 enable；`tools/build-matrix.sh` 对七种基础/URL/DPI/PATH/NO_DEBUG 组合逐项 clean build。这些工具是回归入口，不是常驻用户态控制面。
- `tests/dpi/run-corpus.sh` 是 detector 黑盒 corpus 入口：默认使用两个 IPv4 network namespace，`--ipv6` 则使用两个 IPv6 `/64`，让 fixture 经过 root namespace 的 FORWARD hook，临时安装带 conntrack state match 的防火墙规则，并通过支持双栈地址的 `tools/natflow-dpi-corpus.c` 和 `tools/natflow-dpi-traffic.c` 注入双向 TCP/UDP payload、过滤 4/16 字节 original tuple、断言固定 catalog revision、app/category、`rule_id=0` 和 evidence direction。conntrack match 确保所选地址族不依赖系统已有 NAT/firewall 或 natflow path 开关获得 `nf_conn`；测试要求空 domain ruleset，并修改对应 family 的 forwarding、iptables/ip6tables、DPI enable 和事件统计，只适用于隔离测试环境。最终 PASS 前会核验 DPI 状态、FORWARD 规则、namespace/veth、forwarding 和临时目录均已恢复。IPv6 extension header 不在当前 DPI 支持和测试范围。
...
- `app_id` 继续是唯一分类结果；packet/byte counter 和 detector mask 是 `natflow_t` 内瞬态 context。所需方向始终没有 payload时，context 可以保留到 conntrack 生命周期结束。
...
- MUST 支持用户事件队列空读返回 0，并通过 `poll()` 等待可读。
...
- SHOULD 评估是否需要支持非 DNS Host 值、IDNA U-label 转换或 IPv6 literal Host；这些都会改变现有行为或 ABI。
