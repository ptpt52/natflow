...
- `tools/natflow-dpi-reader.c` 是 v3 queue ABI 的可编译参考消费者；`tools/natflow-dpi-ctl-smoke.sh` 在空 ruleset 测试环境验证控制事务并恢复 enable；`tools/build-matrix.sh` 对七种基础/URL/DPI/PATH/NO_DEBUG 组合逐项 clean build。这些工具是回归入口，不是常驻用户态控制面。
